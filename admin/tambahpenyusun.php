...
$ceka=$conn->query("SELECT * FROM `jadwal_penyusun` WHERE `id_jadwal`='$idj' AND `id_asesor`='$ida'");
$jumceka=$ceka->num_rows;
if ($jumceka==0){
	$conn->query("INSERT INTO `jadwal_penyusun`(`id_jadwal`, `id_asesor`) VALUES ('$idj', '$ida')");
		// SMS Asesor
	...
	$mail->MsgHTML($pesan);
	if ($mail->Send()){
	    header("location:media.php?module=jadwalpenyusun&idj=".$_POST['idj']);
	}else{
