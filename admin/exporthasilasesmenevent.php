...
$excelku->getActiveSheet()->getColumnDimension('T')->setWidth(15);
$excelku->getActiveSheet()->getColumnDimension('U')->setWidth(15);
$excelku->getActiveSheet()->getColumnDimension('V')->setWidth(15);
...
$SI->setCellValue('L1', 'PENDIDIKAN TEERAKHIR'); //Kolom Kode Pendidikan
$SI->setCellValue('M1', 'PEKERJAAN'); //Kolom Pekerjaan
$SI->setCellValue('N1', 'KODE JADWAL BNSP'); //Kolom Kode Jadwal
...
$SI->setCellValue('T1', 'INSTITUSI PENGUSUL'); //Kolom Institusi Pengusul
$SI->setCellValue('U1', 'ASAL INSTITUSI'); //Kolom Institusi Pengusul
$SI->setCellValue('V1', 'ALAMAT INSTITUSI'); //Kolom Institusi Pengusul
...
$excelku->getActiveSheet()->setSharedStyle($headerStylenya, "A1 : V1"); */

// Mengambil data dari event
$sqljadwaltuk2="SELECT * FROM `jadwal_asesmen` WHERE `id_event`='$_GET[tgas]'";
$getevent=$conn->query($sqljadwaltuk2);
...
while ($gjdw=$getevent->fetch_assoc()){
	...
	$strsql	= "SELECT * from `asesi_asesmen` WHERE `id_jadwal`='$gjdw[id]'";
	...
	while ($row = $res->fetch_assoc()) {
		...
		$getasesor=$conn->query("SELECT * FROM `jadwal_asesor` WHERE `id_jadwal`='$row[id_jadwal]'");
		...
		$sqlgetdatawilayah="SELECT * FROM `data_wilayah` WHERE `id_wil`='$dtas[kota]'";
		$getdatawilayah=$conn->query($sqlgetdatawilayah);
		$dtwilayah=$getdatawilayah->fetch_assoc();
		$kota=trim($dtwilayah['nm_wil']);
		$SI->setCellValue("H".$baris,$kota); //mengisi data untuk kota
		
		$sqlgetdatawilayah="SELECT * FROM `data_wilayah` WHERE `id_wil`='$dtas[propinsi]'";
		$getdatawilayah=$conn->query($sqlgetdatawilayah);
		$dtwilayah=$getdatawilayah->fetch_assoc();
		$propinsi=trim($dtwilayah['nm_wil']);
		...
		$SI->setCellValue("L".$baris,$gpdd['jenjang_pendidikan']); //mengisi data untuk pendidikan
		$sqlgetpkj="SELECT * FROM `pekerjaan` WHERE `id`='$dtas[pekerjaan]'";
		$getpkj=$conn->query($sqlgetpkj);
		$gpkj=$getpkj->fetch_assoc();
		$SI->setCellValue("M".$baris,$gpkj['pekerjaan']); //mengisi data untuk pekerjaan
		...
		$sqlgetsba="SELECT * FROM `sumber_anggaran` WHERE `id`='$dtjadwal[sumber_anggaran]'";
		$getsba=$conn->query($sqlgetsba);
		$gsba=$getsba->fetch_assoc();
		$SI->setCellValue("Q".$baris,$gsba['jenis_anggaran']); //mengisi data untuk sumber anggaran
		$sqlgetpsba="SELECT * FROM `pemberi_anggaran` WHERE `id`='$dtjadwal[pemberi_anggaran]'";
		$getpsba=$conn->query($sqlgetpsba);
		$gpsba=$getpsba->fetch_assoc();
		$SI->setCellValue("R".$baris,$gpsba['nama_instansi']); //mengisi data untuk pemberi anggaran
		$SI->setCellValue("S".$baris,$row['status_asesmen']); //mengisi data untuk status asesmen
		$sqlgetsbap="SELECT * FROM `pengusul` WHERE `no_pendaftaran`='$dtas[id_pengusul]'";
		$getsbap=$conn->query($sqlgetsbap);
		$gsbap=$getsbap->fetch_assoc();
		$SI->setCellValue("T".$baris,$gsbap['nama_kantor']); //mengisi data untuk status asesmen
		$SI->setCellValue("U".$baris,$dtas['nama_kantor']); //mengisi data untuk institusi asal tempat kerja
		$SI->setCellValue("V".$baris,$dtas['alamat_kantor']); //mengisi data untuk institusi asal tempat kerja
