...
$ceka=$conn->query("SELECT * FROM `asesor_verifikatorform` WHERE `id_verifikatorform`='$idadel' AND `id_jadwal`='$idjdel'");
...
if ($jumceka!=0){
		...
		$isismsasr="Yth. ".$hpasr['nama'].", Tugas Verifikasi TUK pada ".$_POST['tgl_verifikasidel']." DIBATALKAN, info lengkap lihat di http://".$urldomain."/asesor";
		...
		$tglasesmen=$_POST['tgl_verifikasidel'];
		// Kirim email dalam format HTML ke Pendaftar
		    $pesan ="Yth. ".$hpasr['nama'].", penjadwalan Anda sebagai Verifikator TUK pada Asesmen Skema $gs[kode_skema] - $gs[judul]<br /><br />  
			    ...
			    $subjek="Penjadwalan Anda sebagai Verifikator TUK pada $tglasesmen DINYATAKAN DIBATALKAN";
	...
	// $mail->MsgHTML($pesan);
	// if ($mail->Send()){
	//     header("location:media.php?module=verifikatorform&idj=".$_POST['idjdel']);
	// }else{
	...
	$conn->query("DELETE FROM `asesor_verifikatorform` WHERE `id_verifikatorform`='$idadel' AND `id_jadwal`='$idjdel'");
	header("location:media.php?module=validatorform&idj=".$_POST['idjdel']);
}
?>