...
$ceka=$conn->query("SELECT * FROM `asesor_verifikatorform` WHERE `id_verifikatorform`='$ida' AND `id_jadwal`='$idj'");
$jumceka=$ceka->num_rows;
if ($jumceka==0){
	$conn->query("INSERT INTO `asesor_verifikatorform`(`id_verifikatorform`, `id_jadwal`) VALUES ('$ida', '$idj')");	
		// SMS Asesor
		...
		$isismsasr="Yth. ".$hpasr['nama'].", Anda dijawal untuk Tugas Verifikasi Form pada ".$_POST['tgl_verifikasiform'].", info lengkap lihat di http://".$urldomain."/asesor";
		...
		$tglasesmen=$_POST['tgl_verifikasiform'];
		// Kirim email dalam format HTML ke Pendaftar
		    $pesan ="Yth. ,".$hpasr['nama']." Anda dijadwalkan sebagai Verifikator Form pada Asesmen Skema $gs[kode_skema] - $gs[judul]<br /><br />  
		            Tanggal: $tglasesmen <br />
			    Skema: $gs[kode_skema] - $gs[judul]<br />
		    		<br /><br />Untuk info selengkapnya silahkan lihat jadwal verifikasi Form di laman Dashboard Anda di http://$urldomain/asesor.<br /><br />";
		    
			    $subjek="Anda dijadwal sebagai Verifikator Form pada $tglasesmen";
	...
	$mail->MsgHTML($pesan);
	if ($mail->Send()){
	    header("location:media.php?module=verifikatorform&idj=".$_POST['idj']);
	}else{
