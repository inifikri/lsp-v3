...
$ceka=$conn->query("SELECT * FROM `asesi_asesmen` WHERE `id_jadwal`='$idjdel'");
...
if ($jumceka>0){
		...
		$isismsasr="Yth. ".$hpasr['nama'].", Tugas Meninjau Instrumen Asesmen (FR.IA.11) pada ".$_POST['tgl_verifikasidel']." DIBATALKAN, info lengkap lihat di http://".$urldomain."/asesor";
		...
		// Kirim email dalam format HTML ke Pendaftar
		    $pesan ="Yth. $namanya, penjadwalan Anda sebagai Peninjau Instrumen Asesmen (FR.IA.11) pada Asesmen Skema $gs[kode_skema] - $gs[judul]<br /><br />  
			    ...
			    $subjek="Penjadwalan Anda sebagai Peninjau Instrumen Asesmen (FR.IA.11) pada $tglasesmen DINYATAKAN DIBATALKAN";
	...
	$mail->MsgHTML($pesan);
	if ($mail->Send()){
	    header("location:media.php?module=peninjauia11&idj=".$_POST['idjdel']);
	}else{
	...
	$conn->query("UPDATE `asesi_asesmen` SET `peninjau_ia11` = NULL WHERE `id_jadwal`='$idjdel'");
	header("location:media.php?module=peninjauia11&idj=".$_POST['idjdel']);
}
?>