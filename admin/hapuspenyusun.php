...
$ceka=$conn->query("SELECT * FROM `jadwal_penyusun` WHERE `id_jadwal`='$idjdel' AND `id_asesor`='$idadel'");
...
if ($jumceka!=0){
		...
		$isismsasr="Yth. ".$hpasr['nama'].", Tugas Asesor pada ".$_POST['tgl_asesmendel']." DIBATALKAN, info lengkap lihat di http://".$urldomain."/asesor";
		...
		$pukulasesmen=$_POST['jam_asesmendel'];
		// Kirim email dalam format HTML ke Pendaftar
		    $pesan ="Yth. $namanya, penjadwalan Anda sebagai Asesor pada Asesmen Skema $gs[kode_skema] - $gs[judul]<br /><br />  
		            Tanggal: $tglasesmen <br />
		            Waktu: Pukul $pukulasesmen <br />
		            Tempat: $tempatasesmen <br />
			    Skema: $gs[kode_skema] - $gs[judul]<br />
		    		<br />DINYATAKAN DIBATALKAN<br />Untuk info selengkapnya silahkan lihat jadwal asesmen di laman Dashboard Anda di http://$urldomain/asesor.<br /><br />";
		    
			    $subjek="Penjadwalan Anda sebagai Asesor pada Asesmen pada $tglasesmen DINYATAKAN DIBATALKAN";
	...
	$conn->query("DELETE FROM `jadwal_penyusun` WHERE `id_jadwal`='$idjdel' AND `id_asesor`='$idadel'");
	...
	$mail->MsgHTML($pesan);
	if ($mail->Send()){
	    header("location:media.php?module=jadwalpenyusun&idj=".$_POST['idjdel']);
	}else{
	...
	//header("location:media.php?module=jadwalasesor&idj=".$_POST['idjdel']);
}
?>