...
$ceka=$conn->query("SELECT * FROM `asesor_verifikatortuk` WHERE `id_asesor`='$idadel' AND `id_jadwal`='$idjdel'");
...
if ($jumceka!=0){
		...
		// Kirim email dalam format HTML ke Pendaftar
		    $pesan ="Yth. $namanya, penjadwalan Anda sebagai Verifikator TUK pada Asesmen Skema $gs[kode_skema] - $gs[judul]<br /><br />  
	...
	$mail->MsgHTML($pesan);
	if ($mail->Send()){
	    header("location:media.php?module=verifikatortuk&idj=".$_POST['idjdel']);
	}else{
	...
	$conn->query("DELETE FROM `asesor_verifikatortuk` WHERE `id_asesor`='$idadel' AND `id_jadwal`='$idjdel'");
	header("location:media.php?module=verifikatortuk&idj=".$_POST['idjdel']);
}
?>