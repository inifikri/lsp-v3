...
$ceka=$conn->query("SELECT * FROM `asesi_asesmen` WHERE `id_jadwal`='$idj'");
$jumceka=$ceka->num_rows;
if ($jumceka>0){ 
	$conn->query("UPDATE `asesi_asesmen` SET `peninjau_ia11`='$ida' WHERE `id_jadwal`='$idj'");
		// SMS Asesor
		...
		$isismsasr="Yth. ".$hpasr['nama'].", Anda dijawal untuk Meninjau Instrumen Asesmen (FR.IA.11) pada ".$_POST['tgl_verifikasi'].", info lengkap lihat di http://".$urldomain."/asesor";
		...
		// Kirim email dalam format HTML ke Pendaftar
		    $pesan ="Yth. $namanya, Anda dijadwalkan sebagai Peninjau Instrumen Asesmen (FR.IA.11) pada Asesmen Skema $gs[kode_skema] - $gs[judul]<br /><br />  
		            Tanggal: $tglasesmen <br />
		            Tempat (TUK): $tempatasesmen <br />
			    Skema: $gs[kode_skema] - $gs[judul]<br />
		    		<br /><br />Untuk info selengkapnya silahkan lihat jadwal Peninjau Asesmen di laman Dashboard Anda di http://$urldomain/asesor.<br /><br />";
		    
			    $subjek="Anda dijadwal sebagai Peninjau Instrumen Asesmen (FR.IA.11) pada $tglasesmen";
	...
	$mail->MsgHTML($pesan);
	if ($mail->Send()){
	    header("location:media.php?module=peninjauia11&idj=".$_POST['idj']);
	}else{
