...
elseif ($_GET['module'] == 'syarat') {
	...
	if (isset($_REQUEST['hapusdocasesi'])) {
		...
		// UPDATE @FHM-PPM 28 JULY 2023 : FUNGSI HAPUS FILE KETIKA HAPUS DATA PERSYARATAN DOKUMEN
		$qfile = $result->fetch_assoc();
		$namafile = $qfile['file'];
		$lokasifile = "foto_asesi/" . $namafile;
		unlink($lokasifile);
		if ($result->num_rows != 0) {
	...
	if (isset($_REQUEST['daftarasesmen'])) {
		...
		if ($row_cnt == 0) {
			...
			Anda Telah Berhasil mendaftarkan diri pada skema ini<b></b></div>";
				...
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_skema`,`id_asesi`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`) VALUES ('$iddokumen','$_GET[id]','$_SESSION[namauser]','$escaped_url','FR-APL-01. FORMULIR PERMOHONAN SERTIFIKASI KOMPETENSI','$rowAgen[nama]','$file','$alamatip')";
...
?>
			<h4>Input Persyaratan Uji Kompetensi</h4>
			<?php
			if($jumpm < $jumpsdev){
				echo "<div class='col-md-12'><div class='alert alert-success alert-dismissable'>
                    ...
                    Anda sudah berhasil melakukan pendaftaran. Tunggu pengecekan dan persetujuan dari administrator
                  </div>";
				...
				echo "<span>Masukkan data persyaratan/pengalaman/portfolio Anda, beserta informasi yang dibutuhkan, kemudian klik <b><a class='btn btn-primary btn-xs'>Tambahkan</a></b></span> 
					<div class='row'>
						<div class='box-body'>
						  <div class='col-md-12'>
			...
			?></select>
				...
				<div class='col-md-12'>
								...
								<div class='modal-content'>
									...
									<div class='modal-body'>
										...
										<table id='example1' class='table table-bordered table-striped'>
											...
											<tbody>" ; $sqlasesidocx="SELECT * FROM `asesi_doc` WHERE `id_asesi`='$as[no_pendaftaran]' ORDER BY `id` DESC" ; $asesidocx=$conn->query($sqlasesidocx);
							while ($pmx = $asesidocx->fetch_assoc()) {
							echo "<tr>
								<td width='50%'>Nama Dokumen : $pmx[nama_doc]<br>No. Dokumen : <b><a href='#myModalM" . $pmx[' id'] . "' data-toggle='modal' data-id='" . $pmx['id'] . "'>$pmx[nomor_doc]</a></b><br>Tanggal Dok.: <b>" . tgl_indo($pmx['tgl_doc']) . "</b></td>
													<td width='50%'>
														<form role='form' method='POST' enctype='multipart/form-data'>
														...
														<div class='col-md-12'>
														<label>Jenis Persyaratan</label>" ; $sqlselectkat="SELECT * FROM `skema_persyaratan` WHERE `id_skemakkni`='$_GET[id]' ORDER BY `id` ASC" ; $tampilkat=$conn->query($sqlselectkat);
											while ($rr = $tampilkat->fetch_assoc()) {
					...
					<div class='col-md-12'>
						<button type='submit' name='addfromlib' class='btn btn-primary btn-block'>Gunakan</button>
					</div>
					</form>
					</td>
					</tr>";
		...
		<?php
			...
			while ($pm = $asesidoc->fetch_assoc()) {
				...
				if ($cekasesiasesmen['status'] == 'P') {
					...
					if ($pm['status'] == 'P') {
						...
						// 	<input type='submit' name='hapusdocasesi' class='btn btn-danger btn-xs' onclick='return confirmation()' title='Hapus' value='$pm[id]'>";
			...
			// UPDATE @FHM-PPM 28 JULY 2023 : Cek data file ttd
		...
		?>
			...
			if (!empty($cekasesiasesmen)) {
				echo "<div class='pull-right'><input type='submit' class='btn btn-success' name='daftarasesmen' value='Simpan Perubahan'></div>";
			}
			echo "</div>
					</form>";
		}
	...
	if (isset($_REQUEST['daftarasesmen'])) {
		...
		if ($row_cnt == 0) {
			...
			Anda Telah Berhasil mendaftarkan diri pada skema ini<b></b></div>";
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`,`id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`) VALUES ('$iddokumen','$_SESSION[namauser]','$_SESSION[id]''$escaped_url','FR-APL-01. FORMULIR PERMOHONAN SERTIFIKASI KOMPETENSI','$rowAgen[nama]','$file','$alamatip')";
				$conn->query($sqlinputdigisign);
				echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Pendaftaran Asesmen Sukses</h4>
									...
									<?php } else { ?>
										...
										<div class='row'>
											...
											Anda Telah Berhasil Mengunggah Bukti <b>Kriteria Unjuk Kerja $el[elemen_kompetensi]</b></div>"; */
																	...
																	echo "<script>alert('Anda Telah Berhasil Mengunggah Bukti Kriteria Unjuk Kerja $el[elemen_kompetensi]'); window.location = '../../lsp/media.php?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$_GET[idj]';</script>";
