...
<head>
	...
	<title>
		<?php
		$queryskema = "SELECT * FROM skema_kkni WHERE id=$_GET[a]";
		$connqueryskema = $conn->query($queryskema);
		$arrqueryskema = $connqueryskema->fetch_assoc();
		?>
		Detail Skema <?= $arrqueryskema['judul'] ?>
	</title>
	<meta name="description" content="<?php echo "$arrqueryskema[judul]"; ?>">
...
<body>
			...
			<div class="pageContent">
				...
				<div class="page-title title-1">
					<div class="container">
						<div class="row">
							<br /><br /><br />
							<h1>Detail Skema <?php echo "$arrqueryskema[judul]" ?></h1>
							<h3>Persyaratan Skema - Unit Kompetensi - Elemen Kompetensi</h3>

							<div class="breadcrumbs">
								<a href="index.php">Beranda</a><i class="fa fa-long-arrow-right main-color"></i><span>Detail Skema</span>
							</div>
				...
				<div class="section">
					<div class="container">
						<div class="row">
							<?php
							$qpersyaratanskema = "SELECT * FROM skema_persyaratan WHERE id_skemakkni='$_GET[a]'";
							$connpersyaratanskema = $conn->query($qpersyaratanskema);
							?>
							<div class='col-md-12 vertical-icons'>
								<div class='heading sub-head'>
									<h3 class='head-4 uppercase font-30'><span class='main-color'>Persyaratan Skema</span></h3>
								</div>
								<ul>
									<?php while ($ps = $connpersyaratanskema->fetch_assoc()) {
										echo "<li style='font-size:16px;'> <i class='fa fa-caret-right' aria-hidden='true'></i> " . $ps['persyaratan'] . "</li>";
									} ?>
								</ul>
							</div>
							<div class='col-md-12 vertical-icons'><br><br></div>
							<div class='col-md-12 vertical-icons'>
								<?php
								$queryunitkopetensi = "SELECT * FROM unit_kompetensi WHERE id_skemakkni='$_GET[a]'";
								$connunitkompetensi = $conn->query($queryunitkopetensi);
								?>
								<div class='heading sub-head'>
									<h3 class='head-4 uppercase font-30'><span class='main-color'>Unit Kompetensi</span></h3>
								</div>
								<div class="accordion">
									<?php $no = 1;
									while ($uk = $connunitkompetensi->fetch_assoc()) {
										$queryelemenkompetensi = "SELECT * FROM elemen_kompetensi WHERE id_unitkompetensi=$uk[id]";
										$connelemenkompetensi = $conn->query($queryelemenkompetensi);
									?>
										<div class="accordion-item">
											<h2 class="accordion-header">
												<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $uk['id']; ?>" aria-expanded="true" aria-controls="collapse<?= $uk['id']; ?>">
													<font style="font-size: 15px;"><?= $no++ . ". " . $uk['judul'] ?></font>
												</button>
											</h2>
											<div id="collapse<?= $uk['id'] ?>" class="accordion-collapse collapse" data-bs-parent="#accordionExample">
												<div class="accordion-body">
													<font style="font-size: 14px;"><b>Elemen Kompetensi :</b></font><br>
													<?php $nomor = 1;
													while ($ek = $connelemenkompetensi->fetch_assoc()) {
														echo $nomor++ . ". <font style='font-size:14px;padding-top:26px;'>" . $ek['elemen_kompetensi'] . "</font><br>";
													?>
													<?php } ?>
												</div>
											</div>
	...
	<script type="text/javascript" src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap/5.3.1/js/bootstrap.min.js"></script>
