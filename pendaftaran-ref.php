...
if ($kodeaman<>$kodeaman2){
	echo "<center>Maaf Kode Keamanan yang Anda masukkan Salah, Silahkan coba <a href='daftarasesi.php' class='btn btn-primary btn-block btn-flat'>Kembali</a></center>";
}else{
...
$sqlcek="SELECT `no_ktp` FROM `asesi` WHERE `no_ktp`='$ktpinput'";
...
if ($ktp==0){
	...
	// identitas pengusul
	$sqlgetpengusul="SELECT * FROM `pengusul` WHERE `no_pendaftaran`='$_POST[id_pengusul]'";
	$getpegusul=$conn->query($sqlgetpengusul);
	$pngs=$getpegusul->fetch_assoc();
    $getbank=$conn->query("SELECT * FROM `rekeningbayar` WHERE `aktif`='Y'");
    // Kirim email dalam format HTML ke Pendaftar
    $pesan ="Terimakasih telah melakukan pendaftaran Uji Kompetensi Keahlian di LSP<br /><br />  
            Nomor Pendaftaran: $gennopendaf <br />
            Nama: $namanyainput <br />
            Nomor Handphone: $no_hpinput <br />
            Kata Sandi (Password): $pass2 <br />
            Instansi Pengusul : $pngs[nama] - $pngs[nama_kantor] <br />
    ...
    $isisms="Yth. $namanyainput Pendaftaran Anda berhasil, No. Pendaftaran Anda adalah $gennopendaf, Password : $pass2 Silahkan masuk/Login ke http://".$urldomain."/asesi";
    if (strlen($no_hp)>8){
    ...
    <head>
      ...
      <title>Bukti Pendaftaran Sistem Informasi Lembaga Sertifikasi Profesi</title>
          ...
          <div class='row'>
            <div class='col-xs-12'>
              <h2 class='page-header'>
                ...
                <small class='pull-right'>Tanggal Pendaftaran: $tgl_daftar2</small>
              </h2>
            </div>
            <!-- /.col -->
          </div>
          ...
          <div class='row invoice-info'>
            <div class='col-sm-4 invoice-col'>
              ...
              <address>
                ...
                No. HP : $no_hpinput<br>
                Email : $emailinput<br>
		Institusi Pengusul : $pngs[nama] - $pngs[nama_kantor]
              </address>
            </div>
          </div>
          <!-- /.row -->";
/*             if ($getprod['biaya']!='0'){
          ...
          <div class='row no-print'>
            <div class='col-xs-12'>
                <form action='pendaftaran-print.php' method='post'>
                ...
                <a href='http://".$urldomain."/asesi' target='_blank' class='btn btn-default'><i class='fa fa-sign-in'></i> Masuk</a>
    ...
    echo "<script>alert('Maaf Pendaftaran Anda Gagal, Nomor Induk Kependudukan (Nomor KTP) telah terdaftar sebelumnya'); window.location = 'index.php'</script>";
