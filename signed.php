...
$sqlidentitas2="SELECT * FROM `lsp`";
$identitas2=$conn->query($sqlidentitas2);
$iden2=$identitas2->fetch_assoc();
...
<head>
  ...
  <title>$iden[nama_lsp] | Telusur Dokumen Digital</title>
  ...
  <div class='content-wrapper'>
    <div class='container'>
      ...
      <section class='content-header'>
        <h1>
          $iden[nama_lsp]
          <small>Penelusuran Dokumen Digital Sistem Informasi LSP</small>
        </h1>
        <ol class='breadcrumb'>
          <li><a href='http://$iden[url_domain]'><i class='fa fa-dashboard'></i> Home</a></li>
          <li class='active'>Tandatangan Digital</li>
        </ol>
      </section>
      ...
      <section class='content'>
        <div class='callout callout-success'>
          <h4>Sistem Verifikasi Dokumen Elektronik $iden[nama_lsp]</h4>
		<div align='center'><img src='images/logolsp.jpg' width='200'/></div>
	...
	if (!empty($_GET['id'] && is_numeric($_GET['id']))){
		$getpenerima="SELECT * FROM `logdigisign` WHERE `id`='$_GET[id]'";
		$penerima=$conn->query($getpenerima);
		$pnm=$penerima->fetch_assoc();
		$ip = $pnm['ip'];
		$details = json_decode(file_get_contents("http://ipinfo.io/{$ip}/json"));
		echo $details->city; // -> "Mountain View"
		echo "<h3 class='box-title'>Penelusuran Dokumen Digital <em>(Digital Signature Tracking)</em></h3>
          </div>
          <div class='box-body'>
		<label>Nama Penandatangan :</label>
		<p>$pnm[penandatangan]</p>
		<label>Judul/Nama Dokumen :</label>
		<p>$pnm[nama_dokumen]</p>
		<label>Ditandatangani di lokasi :</label>
		<div>Hostname Provider: ".$details->hostname."</div>
		<div>ISP: ".$details->org."</div>
		<div>Negara: ".$details->country."</div>
    		<div>Provinsi: ".$details->region."</div>
    		<div>Kota/Kabupaten: ".$details->city."</div>
    		<div>Lokasi Koordinat: ".$details->loc."</div>
		<label>Ditandatangani pada :</label>
		<p>$pnm[waktu]</p>
		<p>Dokumen ini sah dan telah disetujui (ditandatangani secara elektronik) oleh $iden2[nama_jabatanpimpinan] LSP $pnm[penandatangan] dan tercatat dalam database $iden[nama_lsp]</p>";
	}else{
		echo "<h3 class='box-title'>Dokumen Tidak Ditemukan</h3>
          </div>
          <div class='box-body'>";
