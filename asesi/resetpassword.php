  ...
  <br>
	...
	if( isset( $_REQUEST['reset'] )){
		...
		if ($kodeaman!=$kodeaman2){
			...
			$periksa=$conn->query("SELECT * FROM `asesi` WHERE `no_pendaftaran`='$npm' AND `no_pendaftaran`!=''") ;
			switch ($pertanyaan){
				default :
				...
				case 'nomorktp':
					...
					$smsnya = "Yth. $ds1[nama], Kata sandi baru Anda adalah $generatepassword Silahkan ganti password Anda di Laman http://".$_SERVER['HTTP_HOST']."/asesi";

					if ($jawaban==$ds1['no_ktp']){
						echo "<center>Yth. <b>$ds1[nama]</b>, Kata sandi baru Anda adalah:<br /><font color='red' size='5'><b>$generatepassword</b></font><br />Silahkan ganti password Anda di Laman Asesi<br><a href='index.php' class='btn btn-primary btn-block btn-flat'>Masuk Laman Asesi</a></center>";
						$sql2 = "UPDATE `asesi` SET `password`='$password' WHERE `no_pendaftaran`='$npm' AND `no_pendaftaran`!=''";
						$result2 = $conn->query($sql2);
						$sql3 = "INSERT INTO `outbox` (`DestinationNumber`, `TextDecoded`, `Coding`, `SenderID`, `CreatorID`) VALUES ('$ds1[nohp]', '$smsnya', 'Default_No_Compression', 'MyPhone1', 'MyPhone1')";
...
$dsn=$conn->query("SELECT `id`, `nama`, `gelar_depan`, `gelar_blk` FROM `asesi` ORDER BY `nama` ASC");
?>       </div>
      <div class="form-group has-feedback">
        <input required type="text" name="npm" class="form-control" placeholder="No. Pendaftaran Asesi">
      </div>
<div class="form-group has-feedback">
