...
elseif ($_GET['module'] == 'tukjarakjauh') {
	// Update @FHM 24 Juli 2023 : Cek data asesmen
	$cekasesiasesmen = $conn->query("SELECT * FROM asesi_tukjarakjauh WHERE asesi_id='$_SESSION[namauser]' AND skema_id='$_GET[ids]'")->fetch_assoc();
	...
	// UPDATE @FHM-PPM 31 JULY 2023 : Query status dokumen asesi yang ditolak
	$qstatusdoktolak = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `status`= 'R' ORDER BY `id` DESC";
	$dokasesi = $conn->query($qstatusdoktolak);
	$statusdoktolak = $dokasesi->num_rows;

	// CEK DATA ASESI DOC YANG BUKAN STATUS DITOLAK
	$qasesidocstatus = $conn->query("SELECT * FROM tukjarakjauh_doc WHERE asesi_id='$_SESSION[namauser]' AND skema_id='$_GET[ids]' AND `status` != 'R' ORDER BY id DESC")->num_rows;

	// UPDATE @FHM-PPM 31 JULY 2023 : Query manggil data dokumen asesi
	$sqlasesidoc = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `status` ORDER BY `id` DESC";
	$asesidoc = $conn->query($sqlasesidoc);
	$jumpm = $asesidoc->num_rows;
	...
	if (isset($_REQUEST['tambahdoctuk'])) {
		...
		// Apabila ada file yang diupload
		if (empty($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
			$query = "INSERT INTO `tukjarakjauh_doc`(`asesi_id`, `skema_id`, `syarattuk_id`, `nama_doc`, `tahun_doc`, `tgl_doc`) VALUES ('$_SESSION[namauser]','$_GET[ids]','$_POST[kategori]','$_POST[nama_dokumen]','$_POST[tgl_dokumen]')";
		} else {
			$alamatfile = uploadDoc($file);
			$query = "INSERT INTO `tukjarakjauh_doc`(`asesi_id`, `skema_id`, `syarattuk_id`, `nama_doc`, `tgl_doc`, `file`) VALUES ('$_SESSION[namauser]','$_GET[ids]','$_POST[kategori]','$_POST[nama_dokumen]','$_POST[tgl_dokumen]','$alamatfile')";
		}
		$querycek = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]'";
		$resultc = $conn->query($querycek);
		$row_cnt = $resultc->num_rows;
		$querycekr = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]' AND `status`='R'";
		$resultcr = $conn->query($querycekr);
		$row_cntr = $resultcr->num_rows;
		if ($row_cnt == 0) {
			$conn->query($query);
		...
		Anda Telah Berhasil Mengunggah Data <b>Persyaratan Permohonan TUK Jarak Jauh</b></div>";
			...
			<h4><i class='icon fa fa-check'></i> Perbaikan Data Dokumen Sukses</h4>
			Anda Telah Berhasil Mengunggah Data Perbaikan/Revisi Dokumen <b>Persyaratan Sertifikasi</b></div>
			<script>window.location = '$base_url/media.php?module=tukjarakjauh&id=$_GET[id]&ida=$_SESSION[namauser]'</script>";
			} else {
				echo "<script>alert('Maaf Data Tersebut Sudah Ada'); window.location = '$base_url/media.php?module=tukjarakjauh&id=$_GET[id]&ida=$_SESSION[namauser]'</script>";
			}
		}
	}
	// UPDATE @FHM-PPM 31 JULY 2023 : Fungsi simpan upload dokumen asesi yang direvisi
	if (isset($_REQUEST['ubahdoctuk'])) {
		$cekqfilelama = $conn->query("SELECT * FROM tukjarakjauh_doc WHERE `asesi_id` ='$_SESSION[namauser]' AND `syarattuk_id` ='$_POST[kategori]' AND `skema_id` ='$_GET[ids]'")->fetch_assoc();
		$querycek = "SELECT * FROM `tukjarkakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]'";
		$resultc = $conn->query($querycek);
		$row_cnt = $resultc->num_rows;
		$querycekr = "SELECT * FROM `tukjarkakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]' AND `status`='R'";
		$resultcr = $conn->query($querycekr);
		$row_cntr = $resultcr->num_rows;
		...
		if (empty($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
			...
			unlink($namafilelama);
			$alamatfile = uploadDoc($file);
			$query = "UPDATE `tukjarakjauh_doc` SET `file` = '$alamatfile', `status` = 'P' WHERE `asesi_id` ='$_SESSION[namauser]' AND `syarattuk_id` ='$_POST[kategori]' AND `skema_id` ='$_GET[ids]'";
		}
		if ($row_cnt == 0) {
			$conn->query($query);
			//header('location:../../media.php?module=syarat&id=$_GET[id]');
	...
	if (isset($_REQUEST['daftartuk'])) {
		...
		if ($row_cnt == 0) {
			...
			$modul = $_POST['ukom'];
			// clear data asesmen_unitkompetensi
			$conn->query("DELETE FROM `asesmen_unitkompetensi` WHERE `id_asesi`='$_SESSION[namauser]' AND `id_skemakkni`='$_GET[ids]'");
			for ($i = 0; $i < $mod; $i++) {
				$conn->query("INSERT INTO `asesmen_unitkompetensi`(`id_asesi`, `id_skemakkni`, `id_unitkompetensi`) VALUES ('$_SESSION[namauser]', '$_GET[ids]', '$modul[$i]')");
			}
			//=====================update data usia=========================================================
			$sqlkalkulasiusia = "SELECT * FROM `asesi` WHERE `no_pendaftaran`='$_SESSION[namauser]'";
			$kalkulasiusia = $conn->query($sqlkalkulasiusia);
	...
	if (isset($_REQUEST['updateasesmen'])) {
		if ($_POST['tujuan_sertifikasi'] == "Lainnya") {
			$querydas = $conn->query("UPDATE `asesi_asesmen` SET `tujuan_sertifikasi` ='$_POST[tujuan_sertifikasi]', `tujuan_lainnya` ='$_POST[tujuan_lainnya]' WHERE id_asesi='$_SESSION[namauser]' AND id_skemakkni='$_GET[ids]'");
		} else {
			$querydas = $conn->query("UPDATE `asesi_asesmen` SET `tujuan_sertifikasi` ='$_POST[tujuan_sertifikasi]',`tujuan_lainnya` ='' WHERE id_asesi='$_SESSION[namauser]' AND id_skemakkni='$_GET[ids]'");
		}
		$mod = count($_POST['ukom']);
		$modul = $_POST['ukom'];
		// clear data asesmen_unitkompetensi
	...
	if ($pupr > 0) {
		...
		if ($usia < 101 && $as['pendidikan'] > 1 && $sydokpokok != 'kosong') {
			if ($cekasesiasesmen['status'] == 'A') {
				...
				$tampilkat = $conn->query($sqlselectkat);
				while ($rr = $tampilkat->fetch_assoc()) {
					$sqlcekpersyaratan = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$as[no_pendaftaran]' AND `skema_id`='$_GET[ids]' AND `syarattuk_id`='$rr[id]' AND `status`!='R' ORDER BY `id` DESC";
					$asesidocps = $conn->query($sqlcekpersyaratan);
					$jumpsx = $asesidocps->num_rows;
					$sqlcekpersyaratanp = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$as[no_pendaftaran]' AND `skema_id`='$_GET[ids]' AND `syarattuk_id`='$rr[id]' AND `status`='R' ORDER BY `id` DESC";
					$asesidocpsp = $conn->query($sqlcekpersyaratanp);
					$jumpsxp = $asesidocpsp->num_rows;
					if ($jumpsx == 0) {
						if ($jumpsxp > 0) {
			...
			// $sqlasesidoc = "SELECT * FROM `asesi_doc` WHERE `id_asesi`='$as[no_pendaftaran]' AND `id_skemakkni`='$_GET[id]' AND `status` != 'R' ORDER BY `id` DESC";
			// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
			$sqlasesidoc = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$as[no_pendaftaran]' AND `skema_id`='$_GET[ids]' ORDER BY `id` DESC";
			$asesidoc = $conn->query($sqlasesidoc);
			$jumpm = $asesidoc->num_rows;
			while ($pm = $asesidoc->fetch_assoc()) {
				switch ($pm['status']) {
