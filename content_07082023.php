									...
									<div class='row'>
										...
										?>
											...
											Anda Telah Berhasil Mengunggah Bukti <b>Kriteria Unjuk Kerja $el[elemen_kompetensi]</b></div>"; */
																...
																echo "<script>alert('Anda Telah Berhasil Mengunggah Bukti Kriteria Unjuk Kerja $el[elemen_kompetensi]'); window.location = '$base_url/media.php?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$_GET[idj]';</script>";
															...
															echo "<br><label>Bukti unjuk kerja elemen:</label>";
...
elseif ($_GET['module'] == 'form-ia-05') {
	...
	if (isset($_REQUEST['simpandata'])) {
		...
		if (empty($_POST['signed'])) {
			...
			$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
			$alamatip = $_SERVER['REMOTE_ADDR'];
			$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`) VALUES ('$iddokumen','$_SESSION[namauser]','$_GET[id]'	,'$escaped_url','FR.AI.05.TES TERTULIS PILIHAN GANDA','$rowAgen[nama]','$file','$alamatip')";
			$conn->query($sqlinputdigisign);
			echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Jawaban berhasil disimpan</h4>
