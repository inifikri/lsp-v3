...
elseif ($_GET['module'] == 'syarat') {
	...
	// CEK DATA ASESI DOC YANG BUKAN STATUS DITOLAK
	$qasesidocstatus = $conn->query("SELECT * FROM asesi_doc WHERE id_asesi='$_SESSION[namauser]' AND id_skemakkni='$_GET[id]' AND `status` != 'R' ORDER BY id DESC")->num_rows;
	...
	if (isset($_REQUEST['tambahdocasesi'])) {
		...
		Anda Telah Berhasil Mengunggah Data <b>Persyaratan Sertifikasi</b></div>";
			...
			<script>window.location = '$base_url/media.php?module=syarat&id=$_GET[id]&ida=$_SESSION[namauser]'</script>";
			} else {
				echo "<script>alert('Maaf Data Tersebut Sudah Ada'); window.location = '$base_url/media.php?module=syarat&id=$_GET[id]&ida=$_SESSION[namauser]'</script>";
			}
		}
	}
	// UPDATE @FHM-PPM 31 JULY 2023 : Fungsi simpan upload dokumen asesi yang direvisi
	if (isset($_REQUEST['ubahdocasesi'])) {
		$cekqfilelama = $conn->query("SELECT * FROM asesi_doc WHERE `asesi_doc`.`id_asesi` ='$_SESSION[namauser]' AND `asesi_doc`.`skema_persyaratan` ='$_POST[kategori]' AND `asesi_doc`.`id_skemakkni` ='$_GET[id]'")->fetch_assoc();
		$querycek = "SELECT * FROM `asesi_doc` WHERE `id_asesi`='$_SESSION[namauser]' AND `id_skemakkni`='$_GET[id]' AND `skema_persyaratan`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tahun_doc`='$_POST[tahun_dokumen]' AND `nomor_doc`='$_POST[nomor_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]'";
		$resultc = $conn->query($querycek);
		$row_cnt = $resultc->num_rows;
		$querycekr = "SELECT * FROM `asesi_doc` WHERE `id_asesi`='$_SESSION[namauser]' AND `id_skemakkni`='$_GET[id]' AND `skema_persyaratan`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tahun_doc`='$_POST[tahun_dokumen]' AND `nomor_doc`='$_POST[nomor_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]' AND `status`='R'";
		...
		if (empty($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
			...
			$namafilelama = "foto_asesi/" . $cekqfilelama['file']; //UPDATE @FHM-PPM 31 JULY 2023
			unlink($namafilelama);
			$alamatfile = uploadDoc($file);
			$query = "UPDATE `asesi_doc` SET `file` = '$alamatfile', `status` = 'P' WHERE `asesi_doc`.`id_asesi` ='$_SESSION[namauser]' AND `asesi_doc`.`skema_persyaratan` ='$_POST[kategori]' AND `asesi_doc`.`id_skemakkni` ='$_GET[id]'";
		}
		if ($row_cnt == 0) {
			$conn->query($query);
			//header('location:../../media.php?module=syarat&id=$_GET[id]');
	...
	if (isset($_REQUEST['hapusdocasesi'])) {
		$cekdu = "SELECT * FROM `asesi_doc` WHERE `id`='$_POST[hapusdocasesi]'";
		$result = $conn->query($cekdu);
		if ($result->num_rows != 0) {
			$conn->query("DELETE FROM `asesi_doc` WHERE `id`='$_POST[hapusdocasesi]'");
			echo "<div class='alert alert-danger alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Hapus Data Sukses</h4>
	...
	if (isset($_REQUEST['daftarasesmen'])) {
		...
		if ($_POST['tujuan_sertifikasi'] == "Lainnya") {
			$querydas = "INSERT INTO `asesi_asesmen`(`tujuan_sertifikasi`, `tujuan_lainnya`, `id_asesi`, `id_skemakkni`, `tgl_daftar`,`biaya`,`status_apl02`) VALUES ('$_POST[tujuan_sertifikasi]', '$_POST[tujuan_lainnya]', '$_SESSION[namauser]','$_GET[id]','$tgl_daftar','$_POST[biaya_asesmen]', NULL)";
		} else {
			$querydas = "INSERT INTO `asesi_asesmen`(`tujuan_sertifikasi`, `id_asesi`, `id_skemakkni`, `tgl_daftar`,`biaya`,`status_apl02`) VALUES ('$_POST[tujuan_sertifikasi]', '$_SESSION[namauser]','$_GET[id]','$tgl_daftar','$_POST[biaya_asesmen]', NULL)";
		}
		...
		if ($row_cnt == 0) {
			...
			if (empty($_POST['signed'])) {
				header('location:$base_url/media.php?module=syarat&id=$_GET[id]&ida=$_SESSION[namauser]');
				echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			...
			echo "<script>alert('Maaf Anda sudah terdaftar pada skema ini'); window.location = '$base_url/media.php?module=syarat&id=$_GET[id]&ida=$_SESSION[namauser]'</script>";
		}
	}
	// UDATE @FHM-PUSTI 1 AGUSTUS 2023 
	if (isset($_REQUEST['updateasesmen'])) {
		if ($_POST['tujuan_sertifikasi'] == "Lainnya") {
			$querydas = $conn->query("UPDATE `asesi_asesmen` SET `tujuan_sertifikasi` ='$_POST[tujuan_sertifikasi]', `tujuan_lainnya` ='$_POST[tujuan_lainnya]' WHERE id_asesi='$_SESSION[namauser]' AND id_skemakkni='$_GET[id]'");
		} else {
			$querydas = $conn->query("UPDATE `asesi_asesmen` SET `tujuan_sertifikasi` ='$_POST[tujuan_sertifikasi]',`tujuan_lainnya` ='' WHERE id_asesi='$_SESSION[namauser]' AND id_skemakkni='$_GET[id]'");
		}
		$mod = count($_POST['ukom']);
		$modul = $_POST['ukom'];
		// clear data asesmen_unitkompetensi
		...
		echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Ubah Data Asesmen Berhasil</h4>
			Anda Telah Berhasil Mengubah Data Asesmen<b></b></div>";
	}
	$sqlgetskema = "SELECT * FROM `skema_kkni` WHERE `id`='$_GET[id]'";
	$getskema = $conn->query($sqlgetskema);
	$gs = $getskema->fetch_assoc();
	...
	if ($pupr > 0) {
		...
		if ($usia < 101 && $as['pendidikan'] > 1 && $sydokpokok != 'kosong') {
			if ($jumpsdev == $qasesidocstatus && !empty($cekasesiasesmen)) {
				echo "<div class='col-md-12'><div class='alert alert-success alert-dismissable'>
                    <h4><i class='icon fa fa-check'></i> Anda Sudah Mendaftar</h4>
                    Anda sudah berhasil melakukan pendaftaran.
                  </div>";
			} else {
				if ($statusdoktolak > 0) {
					echo "<div class='col-md-12'><div class='alert alert-warning alert-dismissable'>
                    <h4> <i class='fa fa-info'></i>&nbsp; Ada Dokumen yang ditolak dan perlu direvisi</h4>
                  </div>";
				} else {
				}
				...
				if ($statusdoktolak > 0) {
					echo "<div class='col-md-3'>
								<div class='form-group'>
									<button type='submit' class='btn btn-primary' name='ubahdocasesi'><i class='glyphicon glyphicon-floppy-disk'></i> Simpan Revisi/Perbaikan</button>
									</form>
								</div>
							</div>";
				} else {
								...
								<div class='modal-content'>
									...
									<div class='modal-body'>
										...
										<table id='example1' class='table table-bordered table-striped'>
											...
											<tbody>";
				// $sqlasesidocx = "SELECT * FROM `asesi` WHERE `no_pendaftaran`='$as[no_pendaftaran]' ORDER BY `id` DESC";
				$sqlasesidocx = "SELECT * FROM `asesi_doc` WHERE `id_asesi`='$as[no_pendaftaran]' ORDER BY `id` DESC";
				// var_dump($sqlasesidocx);
			...
			echo "<h1>FR-APL-01. FORMULIR PERMOHONAN SERTIFIKASI KOMPETENSI</h1>
			...
			if (!empty($cekasesiasesmen)) {
				echo "<div class='pull-right'><input type='submit' class='btn btn-success' name='updateasesmen' value='Simpan Perubahan'></div>";
			}
			echo "<h2>Bagian 3: Bukti Kelengkapan Pemohon</h2>
			...
			while ($pm = $asesidoc->fetch_assoc()) {
				...
				if (!empty($pm['file'])) {
					if ($pm['status'] == 'R') {
						echo "<a class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-remove' aria-hidden='true' title='Dokumen Berhasil Diunggah'></span></a>";
					} else {
...
// Bagian Input TUK Jarak Jauh ==========================================================================================================
elseif ($_GET['module'] == 'tukjarakjauh') {
	$url =  "{$_SERVER['HTTP_HOST']}{$_SERVER['REQUEST_URI']}";
	$iddokumen = md5($url);
	// Update @FHM 24 Juli 2023 : Cek data asesmen
	$cekasesiasesmen = $conn->query("SELECT * FROM asesi_tukjarakjauh WHERE asesi_id='$_SESSION[namauser]' AND skema_id='$_GET[ids]' AND jadwal_id='$_GET[idj]'")->fetch_assoc();
	// UPDATE @FHM-PPM 31 JULY 2023 : Query manggil data skema persyaratan
	$sqllsp = "SELECT * FROM `persyaratan_tukjarakjauh` ORDER BY `id` ASC";
	$lsp = $conn->query($sqllsp);
	$jumpsdev = $lsp->num_rows;

	// UPDATE @FHM-PPM 31 JULY 2023 : Query status dokumen asesi yang ditolak
	$qstatusdoktolak = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `status`= 'R' ORDER BY `id` DESC";
	$dokasesi = $conn->query($qstatusdoktolak);
	$statusdoktolak = $dokasesi->num_rows;

	// CEK DATA ASESI DOC YANG BUKAN STATUS DITOLAK
	$qasesidocstatus = $conn->query("SELECT * FROM tukjarakjauh_doc WHERE asesi_id='$_SESSION[namauser]' AND skema_id='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `status` != 'R' ORDER BY id DESC")->num_rows;

	// UPDATE @FHM-PPM 31 JULY 2023 : Query manggil data dokumen asesi
	$sqlasesidoc = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `status` ORDER BY `id` DESC";
	$asesidoc = $conn->query($sqlasesidoc);
	$jumpm = $asesidoc->num_rows;
	...
	function uploadDoc($file)
	{
		//$file_max_weight = 20000000; //Ukuran maksimal yg dibolehkan( 20 Mb)
		$ok_ext = array('jpg', 'png', 'gif', 'jpeg', 'JPG', 'PNG', 'GIF', 'JPEG', 'pdf', 'PDF'); // ekstensi yang diijinkan
		$destination = "foto_tukjarakjauh/"; // tempat buat upload
		$filename = explode(".", $file['name']);
		$file_name = $file['name'];
		$file_name_no_ext = isset($filename[0]) ? $filename[0] : null;
		$file_extension = $filename[count($filename) - 1];
	...
	if (isset($_REQUEST['tambahdoctuk'])) {
		$file = $_FILES['file'];
		// Apabila ada file yang diupload
		if (empty($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
			$query = "INSERT INTO `tukjarakjauh_doc`(`asesi_id`, `skema_id`,`jadwal_id`, `syarattuk_id`, `nama_doc`, `tahun_doc`, `tgl_doc`) VALUES ('$_SESSION[namauser]','$_GET[ids]','$_GET[idj]','$_POST[kategori]','$_POST[nama_dokumen]','$_POST[tgl_dokumen]')";
		} else {
			$alamatfile = uploadDoc($file);
			$query = "INSERT INTO `tukjarakjauh_doc`(`asesi_id`, `skema_id`,`jadwal_id`, `syarattuk_id`, `nama_doc`, `tgl_doc`, `file`) VALUES ('$_SESSION[namauser]','$_GET[ids]','$_GET[idj]','$_POST[kategori]','$_POST[nama_dokumen]','$_POST[tgl_dokumen]','$alamatfile')";
		}
		$querycek = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]'";
		$resultc = $conn->query($querycek);
		$row_cnt = $resultc->num_rows;
		$querycekr = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]' AND `status`='R'";
		$resultcr = $conn->query($querycekr);
		$row_cntr = $resultcr->num_rows;
		if ($row_cnt == 0) {
			$conn->query($query);
		...
		<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
		<h4><i class='icon fa fa-check'></i> Tambah Data Sukses</h4>
		Anda Telah Berhasil Mengunggah Data <b>Persyaratan Permohonan TUK Jarak Jauh</b></div>";
		} else {
			if ($row_cnt > 0) {
				$conn->query($query);
				//header('location:../../media.php?module=syarat&id=$_GET[id]');
			...
			<h4><i class='icon fa fa-check'></i> Perbaikan Data Dokumen Sukses</h4>
			Anda Telah Berhasil Mengunggah Data Perbaikan/Revisi Dokumen <b>Persyaratan Sertifikasi</b></div>
			<script>window.location = '$base_url/media.php?media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]'</script>";
			} else {
				echo "<script>alert('Maaf Data Tersebut Sudah Ada'); window.location = '$base_url/media.php?media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]'</script>";
			}
		}
	}
	// UPDATE @FHM-PPM 31 JULY 2023 : Fungsi simpan upload dokumen asesi yang direvisi
	if (isset($_REQUEST['ubahdoctuk'])) {
		$cekqfilelama = $conn->query("SELECT * FROM tukjarakjauh_doc WHERE `asesi_id` ='$_SESSION[namauser]' AND `syarattuk_id` ='$_POST[kategori]' AND `skema_id` ='$_GET[ids]' AND `jadwal_id` ='$_GET[idj]'")->fetch_assoc();
		$querycek = "SELECT * FROM `tukjarkakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `jadwal_id` ='$_GET[idj]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]'";
		$resultc = $conn->query($querycek);
		$row_cnt = $resultc->num_rows;
		$querycekr = "SELECT * FROM `tukjarkakjauh_doc` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `jadwal_id` ='$_GET[idj]' AND `syarattuk_id`='$_POST[kategori]' AND `nama_doc`='$_POST[nama_dokumen]' AND `tgl_doc`='$_POST[tgl_dokumen]' AND `status`='R'";
		$resultcr = $conn->query($querycekr);
		$row_cntr = $resultcr->num_rows;
		...
		if (empty($_FILES['file']['tmp_name']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
			...
			$namafilelama = "foto_tukjarakjauh/" . $cekqfilelama['file']; //UPDATE @FHM-PPM 31 JULY 2023
			unlink($namafilelama);
			$alamatfile = uploadDoc($file);
			$query = "UPDATE `tukjarakjauh_doc` SET `file` = '$alamatfile', `status` = 'P' WHERE `asesi_id` ='$_SESSION[namauser]' AND `syarattuk_id` ='$_POST[kategori]' AND `skema_id` ='$_GET[ids]' AND `jadwal_id` ='$_GET[idj]'";
		}
		if ($row_cnt == 0) {
			$conn->query($query);
			//header('location:../../media.php?module=syarat&id=$_GET[id]');
		...
		<h4><i class='icon fa fa-check'></i> Tambah Data Sukses</h4>
		Anda Telah Berhasil Mengunggah Data <b>Persyaratan Sertifikasi</b></div>
		<script>window.location = '$base_url/media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]'</script>";
			header("Refresh:0");
		} else {
			if ($row_cnt > 0) {
				$conn->query($query);
			...
			<script>window.location = '$base_url/media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]'</script>";
				...
				echo "<script>alert('Maaf Data Tersebut Sudah Ada'); window.location = '$base_url/media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]'</script>";
			}
		}
	}
	if (isset($_REQUEST['hapusdoctuk'])) {
		$cekdu = "SELECT * FROM `tukjarakjauh_doc` WHERE `id`='$_POST[iddeldoctuk]'";
		$result = $conn->query($cekdu);
		if ($result->num_rows != 0) {
			$conn->query("DELETE FROM `tukjarakjauh_doc` WHERE `id`='$_POST[iddeldoctuk]'");
			$directory = "foto_tukjarakjauh/" . $cekdu['file'];
			unlink($directory);
			echo "<div class='alert alert-danger alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Hapus Data Sukses</h4>
			Anda Telah Berhasil Menghapus Data <b>Persyaratan TUK Persyaratan Jarak Jauh</b></div>";
		} else {
			echo "<script>alert('Maaf Persyaratan Skema dengan tersebut Tidak Ditemukan');</script>";
		}
	}
	if (isset($_REQUEST['daftartuk'])) {
		$tgl_daftar = date("Y-m-d");
		$querydas = "INSERT INTO `asesi_tukjarakjauh`(`asesi_id`,`skema_id`,`jadwal_id`,`tgl_tuk`,`alamat_tuk`,`kepemilikan_tuk`) VALUES ('$_SESSION[namauser]','$_GET[ids]','$_GET[idj]','$tgl_daftar','$_POST[alamat_tuk]','$_POST[kepemilikan_tuk]')";
		$querycek = "SELECT * FROM `asesi_tukjarakjauh` WHERE `asesi_id`='$_SESSION[namauser]' AND `skema_id`='$_GET[ids]' AND `status` != 'A'";
		$resultc = $conn->query($querycek);
		$row_cnt = $resultc->num_rows;
		if ($row_cnt == 0) {
			$conn->query($querydas);
			//Notifikasi Email dan SMS====================================================
			$sqlgetskema = "SELECT * FROM `skema_kkni` WHERE `id`='$_GET[ids]'";
			$getskema = $conn->query($sqlgetskema);
			$gs = $getskema->fetch_assoc();
			$email = $as['email'];
			$namanya = $as['nama'];
			...
			if ($jumlspkonstruksi > 0) {
				...
				$pesan = "Terimakasih telah melakukan permohonan TUK jarak jauh pada Skema $gs[kode_skema] - $gs[judul]<br /><br />  
				ID Asesi: $_SESSION[namauser] <br />
				Nama: $namanya <br />
				Nomor Handphone: $as[nohp] <br />
				Skema: $gs[kode_skema] - $gs[judul]<br /><br />";
				$subjek = "Terimakasih telah melakukan permohonan TUK jarak jauh di $iden[nama_lsp]";
				$dari = "From: noreply@" . $urldomain . "\r\n";
				$dari .= "Content-type: text/html\r\n";
			}
			// Kirim email ke member
			...
			$folderPath = "foto_tandatangan/";
			if (empty($_POST['signed'])) {
				header('location:$base_url/media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]');
				echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Pendaftaran Asesmen Sukses</h4>
			Anda Telah Berhasil mendaftarkan diri pada skema ini<b></b></div>";
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				// $sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`,`id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$_GET[ids]','$escaped_url','FR-TUK.FORMULIR PERMOHONAN TUK JARAK JAUH','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`,`id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$_GET[ids]','$escaped_url','FR-TUK.FORMULIR PERMOHONAN TUK JARAK JAUH','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				header('location:$base_url/media.php?module=tukjarakjauh&id=$_GET[id]&ida=$_SESSION[namauser]');
				echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Pendaftaran Asesmen Sukses</h4>
			Anda Telah Berhasil mendaftarkan diri pada skema ini<b></b><br><b>Tanda Tangan Sukses Diupload</b></div>
			...
			echo "<script>alert('Maaf Anda sudah terdaftar pada skema ini'); window.location = '$base_url/media.php?module=tukjarakjauh&idj=$_GET[idj]&ids=$_GET[ids]'</script>";
		}
	}
	// UDATE @FHM-PUSTI 1 AGUSTUS 2023 
	if (isset($_REQUEST['updateasesituk'])) {
		$querydas = $conn->query("UPDATE `asesi_tukjarakjauh` SET `alamat_tuk` ='$_POST[alamat_tuk]', `kepemilikan_tuk` ='$_POST[kepemilikan_tuk]' WHERE asesi_id='$_SESSION[namauser]' AND skema_id='$_GET[ids]' AND jadwal_id='$_GET[idj]'");
		echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Ubah Data Asesmen Berhasil</h4>
			Anda Telah Berhasil Mengubah Data Asesmen<b></b></div>";
    ...
    <section class='content-header'>
      <h1>
        Persyaratan TUK Jarak Jauh
        <small>Input Data</small>
      </h1>
      <ol class='breadcrumb'>
        <li><a href='media.php?module=home'><i class='fa fa-dashboard'></i> Home</a></li>
        <li><a href='media.php?module=skema'><i class='fa fa fa-edit'></i> Skema Sertifikasi</a></li>
        <li class='active'>Data Persyaratan TUK Jarak Jauh</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class='content'>
	...
	if ($pupr > 0) {
		echo "<div class='panel box box-success'>
		  <div class='box-header with-border'>
			<h4 class='box-title'>
			  <a data-toggle='collapse' data-parent='#accordion' href='#collapseFour'>
				Pendaftaran TUK Jarak Jauh $gs[judul]
			  </a>
			</h4>
		  </div>
		  <div id='collapseFour' class='panel-collapse collapse in'>";
	...
	if ($pupr > 0) {
		...
		if ($cekasesiasesmen['status'] == 'A') {
			echo "<div class='col-md-12'><div class='alert alert-success alert-dismissable'>
                    <h4><i class='icon fa fa-check'></i> Selamat, Permohonan TUK Jarak Jauh Anda Telah Disetujui</h4>
                    Anda sudah berhasil melakukan pendaftaran dan disetujui.
                  </div>";
		} elseif ($jumpsdev == $qasesidocstatus && !empty($cekasesiasesmen)) {
			echo "<div class='col-md-12'><div class='alert alert-success alert-dismissable'>
                    <h4><i class='icon fa fa-check'></i> Anda Sudah Mendaftar</h4>
                    Anda sudah berhasil melakukan pendaftaran.
                  </div>";
			...
			if ($statusdoktolak > 0) {
				echo "<div class='col-md-12'><div class='alert alert-warning alert-dismissable'>
                    <h4> <i class='fa fa-info'></i>&nbsp; Ada Dokumen yang ditolak dan perlu direvisi</h4>";
				while ($r = $dokasesi->fetch_assoc()) {
					$st = $conn->query("SELECT * FROM persyaratan_tukjarakjauh WHERE id='$r[syarattuk_id]'")->fetch_assoc();
					echo "
						<ul>
							<li><b>$st[persyaratan]</b><br>
							<b>Catatan/Keterangan Asesor :</b> $r[catatan_asesor]
							</li>
						</ul>";
				}
				echo "</div>";
						...
						<div class='box-body'>
						  <div class='col-md-12'>
							...
							<div class='form-group'>
								...
								<select name='kategori' class='form-control' id='kategori'>
								<option value=''>--Pilih Kategori Persyaratan--</option>";
			$sqlselectkat = "SELECT * FROM `persyaratan_tukjarakjauh` ORDER BY `id` ASC";
			$tampilkat = $conn->query($sqlselectkat);
			while ($rr = $tampilkat->fetch_assoc()) {
				$sqlcekpersyaratan = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$as[no_pendaftaran]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `syarattuk_id`='$rr[id]' AND `status`!='R' ORDER BY `id` DESC";
				$asesidocps = $conn->query($sqlcekpersyaratan);
				$jumpsx = $asesidocps->num_rows;
				$sqlcekpersyaratanp = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$as[no_pendaftaran]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' AND `syarattuk_id`='$rr[id]' AND `status`='R' ORDER BY `id` DESC";
				$asesidocpsp = $conn->query($sqlcekpersyaratanp);
				$jumpsxp = $asesidocpsp->num_rows;
				if ($jumpsx == 0) {
					if ($jumpsxp > 0) {
			...
			if ($statusdoktolak > 0) {
				echo "<div class='col-md-3'>
								<div class='form-group'>
									<button type='submit' class='btn btn-primary' name='ubahdoctuk'><i class='glyphicon glyphicon-floppy-disk'></i> Simpan Revisi/Perbaikan</button>
									</form>
								</div>
							</div>";
			} else {
				echo "<div class='col-md-3'>
							  <div class='form-group'>
							  <button type='submit' class='btn btn-primary' name='tambahdoctuk'>Tambahkan</button>
							  </form>
							  </div>
							</div>";
			}
		...
		echo
		"<form role='form' method='POST' enctype='multipart/form-data'>
			<h2>Bagian 1: Bukti Kelengkapan Pemohon</h2>
					<h3>Bukti Persyaratan Dasar Pemohon</h3>
					<div class='row'>
					<div class='box-body'>
					<div style='overflow-x:auto;'>
		...
		// $sqlasesidoc = "SELECT * FROM `asesi_doc` WHERE `id_asesi`='$as[no_pendaftaran]' AND `id_skemakkni`='$_GET[id]' AND `status` != 'R' ORDER BY `id` DESC";
		// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
		$sqlasesidoc = "SELECT * FROM `tukjarakjauh_doc` WHERE `asesi_id`='$as[no_pendaftaran]' AND `skema_id`='$_GET[ids]' AND `jadwal_id`='$_GET[idj]' ORDER BY `id` DESC";
		$asesidoc = $conn->query($sqlasesidoc);
		$jumpm = $asesidoc->num_rows;
		while ($pm = $asesidoc->fetch_assoc()) {
			switch ($pm['status']) {
			...
			$prt = $portfolioskpi->fetch_assoc();
			echo "<tr class=gradeX><td>$no</td><td><b>$prt[persyaratan]</b><br>Nama Dokumen : <a href='#myModal" . $pm['id'] . "' data-toggle='modal' data-id='" . $pm['id'] . "'><b>$pm[nama_doc]</b></a><br>Tanggal Dok.: <b>" . tgl_indo($pm['tgl_doc']) . "</b></td><td>";
			if (!empty($pm['file'])) {
				if ($pm['status'] == 'R') {
					echo "<a class='btn btn-danger btn-xs'><span class='glyphicon glyphicon-remove' aria-hidden='true' title='Dokumen Berhasil Diunggah'></span></a>";
				} else {
			...
			if ($cekasesiasesmen['status'] == 'P') {
				...
				if ($pm['status'] == 'P') {
					// echo "<br />
					// 	<input type='hidden' name='iddeldoctuk' value='$pm[id]'>
					// 	<input type='submit' name='hapusdoctuk' class='btn btn-danger btn-xs' onclick='return confirmation()' title='Hapus' value='Hapus'>";

					// UPDATE @FHM-PPM 28 JULY 2023 : Perubahan fungsi hapus persyaratan dokumen form-apl-01
					echo "<br />
							<button type='submit' name='hapusdoctuk' class='btn btn-danger btn-xs' onclick='return confirmation()' title='Hapus Data/Dokumen TUK' value='$pm[id]'>Hapus</button";
				}
			}
			echo "</td></tr>";
			$no++;
						...
						<div class='modal fade' id='myModal" . $pm['id'] . "' tabindex='-1' role='dialog' aria-labelledby='myModalLabel' aria-hidden='true'>
							<div class='modal-dialog'>
								<div class='modal-content'>
									...
									<div class='modal-body'><embed src='foto_tukjarakjauh/$pm[file]' width='100%' height=600px'/>
									</div>
									<div class='modal-footer'>
										<button type='button' class='btn btn-default' data-dismiss='modal'>Tutup</button>
									</div>
		...
		echo "</tbody></table></div>
			<h2>Bagian 2: Data Pemohon TUK Jarak Jauh</h2>
				<div class='row'>";
		if ($cekasesiasesmen['status'] == 'A') {
		} else {
			echo "		<div class='col-md-6'>
							<div class='form-group'>
								<label>Alamat Pelaksanaan TUK</label>
								<input type='text' name='alamat_tuk' class='form-control' placeholder='Alamat Lengkap Pelaksanaan TUK' required>
							</div>
						</div>
						<div class='col-md-6'>
							<div class='form-group'>
								<label>Kepemilikan TUK</label>
									<select name='kepemilikan_tuk' class='form-control' required>
										<option value=''>--Pilih Kepemilikan TUK--</option>
										<option value='Rumah Milik Sendiri'>Rumah Milik Sendiri</option>
										<option value='Rumah Orang Lain'>Rumah Orang Lain</option>
										<option value='Kantor/Tempat Kerja'>Kantor/Tempat Kerja</option>
										<option value='TUK PPM Manajemen'>TUK PPM Manajemen</option>
									</select>
							</div>
						</div>
						</div>";
			if (!empty($cekasesiasesmen)) {
				echo "<div class='pull-right'><input type='submit' class='btn btn-success' name='updateasesituk' value='Simpan Perubahan'></div>";
			}
		}
		echo "<br><br><br>
			<table TUKe id='table-example' class='table table-bordered table-striped'>
				...
				<tbody>
					<tr>
						<td>1.</td>
						<td>$as[nama]</td>
						<td>$cekasesiasesmen[alamat_tuk]</td>
						<td>$cekasesiasesmen[kepemilikan_tuk]</td>
					</tr>
				</tbody>
			</table><br>";
		if ($jumpm == 0) {
		...
		// $sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE `id_dokumen`='$iddokumen' AND `penandatangan`='$namanya' ORDER BY `waktu` DESC";
		// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
		$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$_GET[ids]' AND id_asesi='$_SESSION[namauser]' AND nama_dokumen='FR-TUK.FORMULIR PERMOHONAN TUK JARAK JAUH' AND `penandatangan`='$as[nama]' ORDER BY `waktu` DESC";
		$cektandatangan = $conn->query($sqlcektandatangan);
		$jumttd = $cektandatangan->num_rows;
		$ttdx = $cektandatangan->fetch_assoc();
		if ($jumttd > 0) {
		...
		if (empty($cekasesiasesmen)) {
			...
			<div align='right' class='col-md-6 col-sm-6 col-xs-6'>";
			if ($jumpm == $jumpsdev) {
				echo "<input type='submit' class='btn btn-success' name='daftartuk' value='Proses Pendaftaran'>";
			} else {
				echo "<input type='submit' class='btn btn-success' name='daftartuk' title='Tombol aktif bila semua persyaratan telah diunggah' value='Proses Pendaftaran' disabled>";
			}
			echo "</div>
			</form>";
		}
...
elseif ($_GET['module'] == 'editprofil') {
	...
	echo "</label>
				...
				<div class='col-md-6'>  
					<div class='form-group'>
						<label>Nomor KTP<font style='color:red;'>*</font></label>
	...
	echo "</select>
				...
				<div class='col-md-12'>  
					<div class='form-group'>
						<label>Universitas/ Lembaga Pendidikan<font style='color:red;'>*</font></label>
						<input required type='text'  name='lembaga_pendidikan' value='$rowAgen[lembaga_pendidikan]' class='form-control'>
					</div>
				</div>
				<div class='col-md-9'>  
					<div class='form-group'>
						<label>Jurusan/Program Studi<font style='color:red;'>*</font></label>
						<input required type='text'  name='prodi' value='$rowAgen[prodi]' class='form-control'>
					</div>
				</div>
				<div class='col-md-3'>  
					<div class='form-group'>
						<label>Tahun Lulus<font style='color:red;'>*</font></label>
				...
				<div class='col-md-12'>
					<div class='form-group'>
						<label>Kebangsaan<font style='color:red;'>*</font></label>
	...
	echo "</select>
				...
				<div class='col-md-6'>
					<div class='form-group'>
						<label>Nomor HP<font style='color:red;'>*</font></label>
	...
	echo "</div>
				...
				<div class='col-md-6'>				
					<div class='form-group'>
						<label>Kota<font style='color:red;'>*</font></label>
	...
	echo "</select>
				...
				<div class='col-md-12'>  
					<div class='form-group'>
						<label>Pekerjaan/ Pengalaman Magang<font style='color:red;'>*</font></label>
	...
	echo "</select>
				...
				<div class='col-md-12'>			
					<div class='form-group'>
						<label>Jabatan / Posisi saat magang<font style='color:red;'>*</font></label>
				...
				<div class='col-md-12'>			
					<div class='form-group'>
						<label>Nama Perusahaan<font style='color:red;'>*</font></label>
				...
				<div class='col-md-12'>			
					<div class='form-group'>
						<label>Alamat Perusahaan/Alamat Magang<font style='color:red;'>*</font></label>
				...
				<div class='col-md-6'>			
					<div class='form-group'>
						<label>Telp. Perusahaan/Magang<font style='color:red;'>*</font></label>
				...
				<div class='col-md-6'>			
					<div class='form-group'>
						<label>Fax. Perusahaan/Magang<font style='color:red;'>*</font></label>
				...
				<div class='col-md-12'>			
					<div class='form-group'>
						<label>Email Perusahaan/Magang<font style='color:red;'>*</font></label>
	...
	if ($getasesmen == 0) {
		...
		while ($asas = $cekasesmen->fetch_assoc()) {
					...
					switch ($asas['status_asesmen']) {
						case "P":
							if ($jumlspkonstruksi > 0) {
								...
								$aksiTUKjarakjauh = "<a class='btn btn-warning form-control' href='media.php?module=tukjarakjauh&idj=$asas[id_jadwal]&ids=$asas[id_skemakkni]'>Permohonan TUK Jarak Jauh</a>";
							}
							break;
						case "K":
						...
						case "BK":
							...
							$aksiTUKjarakjauh = "<a class='btn btn-primary form-control' href='media.php?module=tukjarakjauh&idj=$asas[id_jadwal]&ids=$asas[id_skemakkni]'>Permohonan TUK Jarak Jauh</a>";
							break;
						case "TL":
			...
			echo "<tr>
					...
					<td>$aksinya<br>$aksiTUKjarakjauh<br><a href='media.php?module=updatesyarat&id=$asas[id_skemakkni]&ida=$_SESSION[namauser]' class='btn btn-success btn-block'>Lengkapi Persyaratan Pendaftaran APL-01</a>
					</td>
				</tr>";
			$no++;
		}
...
elseif ($_GET['module'] == 'jadwal') {
	...
	while ($pm0 = $asesiikut->fetch_assoc()) {
		...
		if ($jnsapl02['apl02'] == 'elemen') { 
			if ($jjw == 0) {
				echo "<a href='?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$pm0[id_jadwal]' class='btn btn-primary btn-xs btn-block'>Input Asesmen Mandiri (FORM-APL-02)</a><br>";
			} else {
...
elseif ($_GET['module'] == 'banding') {
	...
	if (isset($_REQUEST['simpan'])) {
		...
		if ($cekak01 > 0) {
			...
			if (empty($_POST['signed'])) {
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.AK.04. BANDING ASESMEN','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				echo "<div class='alert alert-success alert-dismissible'>
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
				<h4><i class='icon fa fa-check'></i> Permohonan Banding Anda berhasil disimpan</h4>
	...
	// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.AK.04. BANDING ASESMEN' ORDER BY `waktu` DESC";
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();
	if ($jumttd > 0) {
		echo "<div class='col-md-12'>
								<label class='' for=''>Persetujuan/ Tanda Tangan yang telah Anda berikan:</label>
								<br/>
								<img src='$ttdx[file]' width='400px'/>x
								<br/>
						  </div>";
	} else {
		echo "<div class='col-md-12'>
...
elseif ($_GET['module'] == 'unggahfile2') {
	...
	$cekasesiasesmen = $conn->query("SELECT * FROM asesi_asesmen WHERE id_asesi='$_SESSION[namauser]' AND id_skemakkni='$jd[id_skemakkni]' AND id_jadwal='$_GET[idj]'")->fetch_assoc();

	// UPDATE @FHM-PUSTI : QUERY BUILDING UNTUK TOTAL ELEMEN KOMPETENSI
	$qunitkompetensi = $conn->query("SELECT * FROM `elemen_kompetensi` LEFT JOIN unit_kompetensi ON unit_kompetensi.id=elemen_kompetensi.id_unitkompetensi WHERE unit_kompetensi.id_skemakkni='$jd[id_skemakkni]' ORDER BY `id_unitkompetensi`  ASC")->num_rows;
	$cekjumlahasesiapl02 = $conn->query("SELECT * FROM `asesi_apl02` WHERE id_skemakkni='$jd[id_skemakkni]' AND id_jadwal=$_GET[idj] ORDER BY `id_elemen` ASC")->num_rows;
	$cekjumlahasesiapl02doc = $conn->query("SELECT * FROM `asesi_apl02doc` WHERE id_skemakkni='$jd[id_skemakkni]' AND id_jadwal=$_GET[idj] ORDER BY `id` ASC")->num_rows;
	...
	if (isset($_REQUEST['simpandata'])) {
		...
		// UPDATE @FHM-PUSTI 2 AGUSTUS 2023 : Update data asesi asesmen status Form APL 02
		$ubahdatastatusasesmen = "UPDATE `asesi_asesmen` SET `status_apl02`='P' WHERE `id_asesi`='$_SESSION[namauser]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]'";
		$conn->query($ubahdatastatusasesmen);
		...
		if (empty($_POST['signed'])) {
			...
			<script>window.location = '" . $base_url . "media.php?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$_GET[idj]'</script>";
		} else {
			$image_parts = explode(";base64,", $_POST['signed']);
			$image_type_aux = explode("image/", $image_parts[0]);
			$image_type = $image_type_aux[1];
			...
			$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
			$alamatip = $_SERVER['REMOTE_ADDR'];
			$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`,`id_skema`, `id_asesi`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$jd[id_skemakkni]','$_SESSION[namauser]','$escaped_url','FR.APL.02. ASESMEN MANDIRI','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
			$conn->query($sqlinputdigisign);
			echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Jawaban berhasil disimpan</h4>
	...
	if (isset($_REQUEST['hapusdocasesi'])) {
		...
		$directory = "foto_apl02/" . $getr['file'];
		$delete = "DELETE FROM `asesi_apl02doc` WHERE `id`=$getr[id]";
	...
	<div class='row'>
		<div class='col-md-12'>
			<div class="callout callout-warning">
				<h4>Keterangan</h4>
				<p>Untuk yang bertanda icon <i class="fa fa-check" aria-hidden="true"></i> artinya sudah mengisi semua isian (Kompeten/Belum Kompeten)</p>
				<p>Untuk yang bertanda icon <i class="fa fa-check" aria-hidden="true"></i><i class="fa fa-check" aria-hidden="true"></i> artinya sudah mengupload semua dokumen serta mengisi isian (Kompeten/Belum Kompeten)</p>
			</div>
			<?php if ($cekasesiasesmen['status_apl02'] == 'R') { ?>
				<div class="callout callout-danger">
					<h4>Mohon maaf Asesmen anda tidak dapat dilanjutkan</h4>
					<p>Informasi lebih lanjut bisa hubungi Asesor</p>
				</div>
			<?php } elseif ($cekasesiasesmen['status_apl02'] == 'A') { ?>
				<div class="callout callout-success">
					<h4>Asesmen Anda Dapat Dilanjutkan</h4>
					<p>Informasi lebih lanjut bisa hubungi Asesor</p>
				</div>
			<?php } else { ?>
			<?php } ?>
			...
			<div class='nav-tabs-custom'>
				<ul class="nav nav-tabs">
					<?php while ($unk1 = $getunitkompetensi->fetch_assoc()) {
						// UPDATE @FHM-TSI 11 Juli 2023 : Menghitung jumlah jawaban (kompeten/belum) yang sudah diinput
						$queryjawaban = "SELECT * FROM asesi_apl02 a LEFT JOIN elemen_kompetensi b ON b.id=a.id_elemen WHERE a.id_asesi='$_SESSION[namauser]' AND b.id_unitkompetensi='$unk1[id]'";
						$jmljawaban = $conn->query($queryjawaban);
						$totaljawaban = $jmljawaban->num_rows;

						// UPDATE @FHM-TSI 11 Juli 2023 : Menghitung jumlah dokumen yang sudah diinput
						// $queryasesidoc = "SELECT * FROM asesi_apl02doc a LEFT JOIN elemen_kompetensi b ON b.id=a.id_elemenkompetensi WHERE a.id_asesi='$_SESSION[namauser]' AND b.id_unitkompetensi='$unk1[id]'";
						$queryasesidoc = "SELECT * FROM asesi_apl02doc a LEFT JOIN elemen_kompetensi b ON b.id=a.id_elemenkompetensi WHERE a.id_asesi='$_SESSION[namauser]' AND b.id_unitkompetensi='$unk1[id]' GROUP BY b.id";
						$jmlasesidoc = $conn->query($queryasesidoc);
						$totalasesidoc = $jmlasesidoc->num_rows;

						// UPDATE @FHM-TSI 11 Juli 2023 : Menghitung jumlah elemen kompetensi
						$cekjmlelemenkompetensi = "SELECT * FROM elemen_kompetensi a LEFT JOIN unit_kompetensi b ON b.id=a.id_unitkompetensi WHERE a.id_unitkompetensi='$unk1[id]'";
						$jmlelkompetensi = $conn->query($cekjmlelemenkompetensi);
						$totalelkompetensi = $jmlelkompetensi->num_rows;

						if ($totaljawaban == $totalelkompetensi && $totalasesidoc == $totalelkompetensi) {
							if ($notab == '1') {
					?>
								<li class="active"><a href="#tab_1" data-toggle="tab"><?php echo $notab++ ?> <i class="fa fa-check" aria-hidden="true"></i><i class="fa fa-check" aria-hidden="true"></i></a></li>
							<?php } else { ?>
								<li><a href="#tab_<?php echo $notab; ?>" data-toggle="tab"><?php echo $notab++ ?> <i class="fa fa-check" aria-hidden="true"></i><i class="fa fa-check" aria-hidden="true"></i></a></li>
							<?php }
						} elseif ($totaljawaban == $totalelkompetensi) {
							if ($notab == '1') { ?>
								<li class="active"><a href="#tab_1" data-toggle="tab"><?php echo $notab++ ?> <i class="fa fa-check" aria-hidden="true"></i></a></li>
							<?php } else { ?>
								<li><a href="#tab_<?php echo $notab; ?>" data-toggle="tab"><?php echo $notab++ ?> <i class="fa fa-check" aria-hidden="true"></i></a></li>
							<?php }
						} elseif ($totalasesidoc == $totalelkompetensi) {
							if ($notab == '1') {
							?>
								<li class="active"><a href="#tab_1" data-toggle="tab"><?php echo $notab++ ?> <i class="fa fa-check" aria-hidden="true"></i></a></li>
							<?php } else { ?>
							...
							if ($notab == '1') {
							?>
								<li class="active"><a href="#tab_1" data-toggle="tab"><?php echo $notab++ ?></a></li>
							<?php } else { ?>
								<li><a href="#tab_<?php echo $notab; ?>" data-toggle="tab"><?php echo $notab++ ?></a></li>
					<?php }
						}
					} ?>
				</ul>
				<div class="tab-content">
					<?php while ($unk = $getunitkompetensi1->fetch_assoc()) {
					...
					?>
								...
								<?php } else { ?>
									<div class="tab-pane" id="tab_<?php echo $notab1 ?>">
									<?php } ?>
									<h3 class='box-title text-green'><b>Unit Kompetensi : <?php echo $notab1 . '. ' . $unk['judul']; ?></b></h3>
									<hr>
									<div class='row'>
										<?php while ($el = $getelemen->fetch_assoc()) {
											// Query Kriteria Unit Kerja
										...
										?>
											<div class='col-md-12'>
												<div class='box box-primary'>
													<div class='box-header with-border'>
														<div class='box-body'>
															<h4 class='text-blue'>Elemen Kompetensi : <b><?= $notab1 . '.' . $no2 . '. ' . $el['elemen_kompetensi']; ?></b></h4>
															<label>Kriteria Unjuk Kerja :</label><br>
															<?php while ($kr = $getkriteria->fetch_assoc()) {
																echo $notab1 . '.' . $no2 . '.' . $no3 . '.' . $kr['kriteria'] . '<br>';
																$no3++;
															}
															...
															if ($cekasesiasesmen['status_apl02'] == NULL) {
																// var_dump($sqlcekjawaban0);
																if ($numjjw0 == 0) {
																	echo "<label>
																...
																if ($numjjw0 == 0) {
																	echo "<label>
																		<input type='radio' name='optionsRadios$el[id]' id='options1$el[id]' value='1' disabled>
																		Kompeten &nbsp;&nbsp;&nbsp;
																	</label>
																	<label>
																		<input type='radio' name='optionsRadios$el[id]' id='options2$el[id]' value='0' disabled>
																		Belum Kompeten
																	</label>";
																} else {
																	if ($jjw0['jawaban'] == 1) {
																		echo "<label>
																			<input type='radio' name='optionsRadios$el[id]' id='options1$el[id]' value='1' checked disabled>
																			Kompeten &nbsp;&nbsp;&nbsp;
																		</label>";
																	} else {
																		echo "<label>
																	...
																	if ($jjw0['jawaban'] == 0) {
																		echo "<label>
																			<input type='radio' name='optionsRadios$el[id]' id='options2$el[id]' value='0' checked disabled>
																			Belum Kompeten
																		</label>";
																	} else {
																		echo "<label>
											...
											Anda Telah Berhasil Mengunggah Bukti <b>Kriteria Unjuk Kerja $el[elemen_kompetensi]</b></div>"; */
																...
																echo "<script>alert('Anda Telah Berhasil Mengunggah Bukti Kriteria Untuk Kerja $el[elemen_kompetensi]'); window.location = '$base_url/media.php?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$_GET[idj]';</script>";
																// ==========================================simpan kompeten atau belum kompeten=============================

																//===========================================================================================================
															}
															echo "<br><label>Bukti untuk kerja elemen:</label>";
															...
															$nodokumen = 1;
															// var_dump($getfilebukti);
															while ($fbk = $filebukti->fetch_assoc()) {
																echo "<br><form role='form' method='POST' enctype='multipart/form-data'>Dokumen Elemen $notab1.$no2.-($nodokumen)&nbsp;<a href='#myModal" . $fbk['id'] . "' class='btn btn-success btn-xs' data-toggle='modal' data-id='" . $fbk['id'] . "'><span class='glyphicon glyphicon-zoom-in' aria-hidden='true' title='Lihat/Unduh Dokumen'></span></a>
																	<input type='hidden' name='iddeldocasesi' value='$fbk[id]'>";
																if ($cekasesiasesmen['status_apl02'] == NULL) {
																	echo "<input type='submit' name='hapusdocasesi' class='btn btn-danger btn-xs' onclick='return confirmation()' title='Hapus' value='Hapus'></form><br>";
																} else {
																}
																echo "<script>
																		$(function(){
															...
															if ($cekasesiasesmen['status_apl02'] == NULL) {
																...
																<input type='file' name='file$el[id]' id='file$el[id]' accept='.pdf, image/*' onchange='readURL(this);'>atau<br>
																<a href='#myModalLib" . $el['id'] . "' class='btn btn-primary btn-xs' data-toggle='modal' data-id='" . $el['id'] . "'>Tambahkan dokumen dari <em>Library</em> (pustaka anda)</a><br>
																<button type='submit' class='btn btn-success btn-xs' name='unggahbukti$el[id]'><i class='fa fa-floppy-o'></i> Simpan Data</button><br>
																";
															} else {
																// NULL
															}
															...
															if (isset($_REQUEST[$addfromlibel])) {
																...
																// $id_elemenkompetensix = "id_elemenkompetensi";
																$id_elemenkompetensix = "id_elemenkompetensi" . $kritvar;
																$id_skemakknix = "id_skemakkni" . $kritvar;
																$filex = "file" . $kritvar;
																...
																if ($jjw > 0) {
																	if (isset($_POST['jawaban'])) {
																		$sqlinputjawaban = "UPDATE `asesi_apl02` SET `jawaban`='$_POST[jawaban]' WHERE `id_asesi`='$_SESSION[namauser]' AND `id_elemen`='$el[id]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]'";
																		$conn->query($sqlinputjawaban);
																	} else {
																		$sqlinputjawaban = "UPDATE `asesi_apl02` SET `jawaban`=NULL WHERE `id_asesi`='$_SESSION[namauser]' AND `id_elemen`='$el[id]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]'";
																		$conn->query($sqlinputjawaban);
																	...
																	if (isset($_POST['jawaban'])) {
																		$sqlinputjawaban = "INSERT INTO `asesi_apl02`(`id_asesi`, `id_elemen`, `id_skemakkni`, `id_jadwal`, `jawaban`) VALUES ('$_SESSION[namauser]','$el[id]','$sk[id]','$_GET[idj]','$_POST[jawaban]')";
																		$conn->query($sqlinputjawaban);
																	} else {
																		$sqlinputjawaban = "INSERT INTO `asesi_apl02`(`id_asesi`, `id_elemen`, `id_skemakkni`, `id_jadwal`, `jawaban`) VALUES ('$_SESSION[namauser]','$el[id]','$sk[id]','$_GET[idj]',NULL)";
																		$conn->query($sqlinputjawaban);
															...
															Anda Telah Berhasil Menambahkan Data <b>Bukti Kompetensi dari <em>Library</em></b></div>"; */
																// UPDATE @FHM-PUSTI 11 AGUSTUS 2023 : Simpan data asesi apl 02 untuk menyimpan data jawaban
																echo "<script>alert('Anda Telah Berhasil Mengunggah Bukti Kriteria Untuk Kerja $el[elemen_kompetensi]'); window.location = '$base_url/media.php?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$_GET[idj]';</script>";
																// ==========================================simpan kompeten atau belum kompeten=============================
																echo "<script>alert('Anda Telah Berhasil Menambahkan Bukti Kriteria Unjuk Kerja $el[elemen_kompetensi] dari Library'); window.location = '$base_url/media.php?module=form-apl-02-el&ida=$_SESSION[namauser]&idj=$_GET[idj]';</script>";
															}
															// var_dump($sqlinputjawaban);
												...
												<div class='modal-dialog'>
													<div class='modal-content'>
														...
														<div class='modal-body'>";
															...
															echo "<h4 class=text-blue>Elemen Kompetensi : <b> $notab1 . $no2 . $el[elemen_kompetensi]</b></h4> <label>Kriteria Unjuk Kerja :</label><br>";
															$getkriteria01 = $conn->query($sqlgetkriteria);
															while ($kr01 = $getkriteria01->fetch_assoc()) {
																echo $notab1 . '.' . $no2 . '.' . $no3 . '.' . $kr01['kriteria'] . '<br>';
																$no3++;
															}
															echo "<br><div class='row'><div class='col-lg-4'>";
															$sqlcekjawaban1 = "SELECT * FROM `asesi_apl02` WHERE `id_asesi`='$_SESSION[namauser]' AND `id_elemen`='$el[id]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]'";
															$cekjawaban1 = $conn->query($sqlcekjawaban1);
															$jjw1 = $cekjawaban1->fetch_assoc();
															$numjjw1 = $cekjawaban1->num_rows;
															// if ($cekasesiasesmen['status_apl02'] == NULL) {
															// var_dump($sqlcekjawaban0);
															if ($numjjw1 == 0) {
																echo "
																...
																<option>-- Pilih Jawaban --</option>
																<option value='1'>Kompeten</option>
																<option value='0' selected>Belum Kompeten</option>
																</select>";
																} else {
																	echo "";
																}
															...
															// 	if ($numjjw1 == 0) {
															// 		echo "<label>
															// 				<input type='radio' name='optionsRadios$jjw1[id_elemen]' id='options1$jjw1[id_elemen]' value='1' disabled>
															// 				Kompeten &nbsp;&nbsp;&nbsp;
															// 			</label>
															// 			<label>
															// 				<input type='radio' name='optionsRadios$jjw1[id_elemen]' id='options2$jjw1[id_elemen]' value='0' disabled>
															// 				Belum Kompeten
															// 			</label>";
															// 	} else {
															// 		if ($jjw1['jawaban'] == 1) {
															// 			echo "<label>
															// 					<input type='radio' name='optionsRadios$jjw1[id_elemen]' id='options1$jjw1[id_elemen]' value='1' checked disabled>
															// 					Kompeten &nbsp;&nbsp;&nbsp;
															// 				</label>";
															// 		} else {
															// 			echo "<label>
															...
															// 		if ($jjw1['jawaban'] == 0) {
															// 			echo "<label>
															// 					<input type='radio' name='optionsRadios$jjw1[id_elemen]' id='options2$jjw1[id_elemen]' value='0' checked disabled>
															// 					Belum Kompeten
															// 				</label>";
															// 		} else {
															// 			echo "<label>
															...
															// KRITERIA UNTUK KERJA 
															echo "<div style='overflow-x:auto; overflow-y:auto;'><br><br>";
															...
															while ($pmx0 = $asesidocx->fetch_assoc()) {
																...
																$cekdirectory = "foto_apl02/" . $pmx['file'];
																if (file_exists($cekdirectory)) {
																	echo "<tr>
																				<td>
																					<div class='col-md-4'>
																	...
																	echo "<tr><td><center><b>Belum Ada Fiel Yang di Upload</b></center></td></tr>";
																}
																/*echo "<script>
																			$(function(){
																						$(document).on('click','.edit-record',function(e){
								...
								<?php } else { ?>
									<div class="box-body">
										<font color="red">Maaf Anda belum memilih Unit Kompetensi saat mendaftar ke Skema</font>
									</div>
							<?php }
							$notab1++;
						} ?>
	...
	if ($cekasesiasesmen['status_apl02'] == NULL) {
		...
		if ($cekjumlahasesiapl02 < $qunitkompetensi && $cekjumlahasesiapl02doc < $qunitkompetensi) {
			echo "<p><b><font color='red'>*</font>Lengkapi terlebih dahulu formulir asesmen</b></p>
								<button type='submit' class='btn btn-success' name='simpandata' disabled>Simpan Jawaban</button>";
		} elseif ($cekjumlahasesiapl02 < $qunitkompetensi) {
			echo "<p><b><font color='red'>*</font>Lengkapi terlebih dahulu formulir asesmen</b></p>
								<button type='submit' class='btn btn-success' name='simpandata' disabled>Simpan Jawaban</button>";
		} elseif ($cekjumlahasesiapl02doc < $qunitkompetensi) {
			echo "<p><b><font color='red'>*</font>Lengkapi terlebih dahulu formulir asesmen</b></p>
								<button type='submit' class='btn btn-success' name='simpandata' disabled>Simpan Jawaban</button>";
		} else {
			echo "<button type='submit' class='btn btn-success' name='simpandata'>Simpan Jawaban</button>";
		}
	} else {
		$sqlasesi = "SELECT * FROM `asesi` WHERE `no_pendaftaran`='$_SESSION[namauser]'";
		$getasesi = $conn->query($sqlasesi);
		...
		if ($ukom > 0) {
			...
			while ($el = $getelemen->fetch_assoc()) {
				...
				while ($kr = $getkriteria->fetch_assoc()) {
					...
					echo "<br><label>Bukti untuk kerja:</label><br>";
...
elseif ($_GET['module'] == 'form-fr-ak-01') {
	...
	if (isset($_REQUEST['simpanformak01'])) {
		$cekak01 = $cekgetak01->num_rows;
		$tglsekarang = date("Y-m-d");
		if ($cekak01 > 0) {
			$folderPath = "foto_tandatangan/";
			if (empty($_POST['signed'])) {
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.AK.01.FORMULIR PERSETUJUAN ASESMEN DAN KERAHASIAAN','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				echo "<div class='alert alert-success alert-dismissible'>
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
				<h4><i class='icon fa fa-check'></i> Persetujuan Anda berhasil disimpan</h4>
			...
			if (empty($_POST['signed'])) {
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$_GET[id]','$escaped_url','FR.AK.01.FORMULIR PERSETUJUAN ASESMEN DAN KERAHASIAAN','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				echo "<div class='alert alert-success alert-dismissible'>
				<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
				<h4><i class='icon fa fa-check'></i> Persetujuan Anda berhasil disimpan</h4>
	...
	// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND nama_dokumen='FR.AK.01.FORMULIR PERSETUJUAN ASESMEN DAN KERAHASIAAN' ORDER BY `waktu` DESC";
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();
	$formak01 = $cekgetak01->fetch_assoc();
	if ($formak01['persetujuan_asesi'] == NULL || $jumttd == 0) {
		echo "<div class='col-md-12'>
							<label class='' for=''>Tanda Tangan:</label>
							<br/>
							<div id='sig' ></div>
		...
		echo "<div class='box-footer'>
							...
							<div class='col-md-4 col-sm-12 col-xs-12'>
									<a href='asesor/form-fr-ak-01.php?ida=$_SESSION[namauser]&idj=$_GET[idj]' class='btn btn-primary form-control' target='_blank'>Unduh Formulir</a>
							</div>
							<div class='col-md-4 col-sm-12 col-xs-12'>
									<button type='submit' class='btn btn-success form-control' name='simpanformak01'>Simpan Jawaban</button>
							</div>
						</div>";
	} else {
		if ($jumttd > 0) {
...
$asr=$getasesordata->fetch_assoc(); */
	...
	if (isset($_REQUEST['simpan'])) {
		...
		if (empty($_POST['signed'])) {
			...
			$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
			$alamatip = $_SERVER['REMOTE_ADDR'];
			$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.AK.02. FORMULIR REKAMAN ASESMEN KOMPETENSI','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
			$conn->query($sqlinputdigisign);
			/* //unit kompetensi
			$sqlgetunitkompetensi0="SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$sk[id]'";
			$getunitkompetensi0=$conn->query($sqlgetunitkompetensi0);
	...
	// $sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE `id_dokumen`='$iddokumen' ORDER BY `id` DESC";
	// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.AK.02. FORMULIR REKAMAN ASESMEN KOMPETENSI' ORDER BY `waktu` DESC";
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();
	...
	$sqlcektandatanganasesor = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$asr[no_ktp]' AND `penandatangan`='$asr[nama]' AND nama_dokumen='FR.AK.02. FORMULIR REKAMAN ASESMEN KOMPETENSI' AND `id_jadwal`='$_GET[idj]' ORDER BY `waktu` DESC";
	...
	if ($jumttd > 0) {
		echo "<div class='col-md-12'>
								<label class='' for=''>Persetujuan/ Tanda Tangan yang telah diberikan : </label>
								<br/>
								<b>Asesi : </b>
								<img src='$ttdx[file]' width='400px'/>
								<b>Asesor : </b>
								<img src='$ttdasesor[file]' width='400px'/>
								<br/>
						  </div>";
		echo "<div class='box-footer'>
					<div class='col-md-4 col-sm-12 col-xs-12'>
...
elseif ($_GET['module'] == 'form-fr-ak-04') {
	...
	$tukjen = $tukjenis->fetch_assoc();
	$noasr = 1;
	$sqlcekgetak04="SELECT * FROM `asesmen_ak04` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' AND `id_skemakkni`='$sk[id]'";
	$getak04=$conn->query($sqlcekgetak04);
	$arrak04=$getak04->fetch_assoc();
	$getasesor = $conn->query("SELECT * FROM `jadwal_asesor` WHERE `id_jadwal`='$_GET[idj]'");
	while ($gas = $getasesor->fetch_assoc()) {
		$sqlasesor = "SELECT * FROM `asesor` WHERE `id`='$gas[id_asesor]'";
		$asesor = $conn->query($sqlasesor);
    ...
    <section class='content-header'>
      <h1>
	  Input FR.AK.04. FORMULIR BANDING ASESMEN
        <small>Skema $sk[judul]</small>
      </h1>
      <ol class='breadcrumb'>
        <li><a href='media.php?module=home'><i class='fa fa-dashboard'></i> Home</a></li>
        <li class='active'>FR.AK.04</li>
      </ol>
    </section>";
	if (isset($_REQUEST['simpan'])) {
		$tglsekarang = date("Y-m-d");
		...
		$folderPath = "foto_tandatangan/";
		if (empty($_POST['signed'])) {
				$sqlcekgetak01b="SELECT * FROM `asesmen_ak04` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' AND `id_skemakkni`='$sk[id]'";
				$cekgetak01b=$conn->query($sqlcekgetak01b);
				$jumak02unit=$cekgetak01b->num_rows;
				if ($jumak02unit>0){
					// update data
					$sqlinputak01="UPDATE `asesmen_ak04` SET alasan_banding='$_POST[alasan_banding]',pertanyaan_1='$_POST[pertanyaan_1]',pertanyaan_2='$_POST[pertanyaan_2]',pertanyaan_3='$_POST[pertanyaan_3]',`tanggal`='$tglsekarang' WHERE `id_asesi`='$_GET[ida]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]'";
					$conn->query($sqlinputak01);
				}else{
					// input data
					$sqlinputak01="INSERT INTO `asesmen_ak04`(`id_asesi`, `id_skemakkni`, `id_jadwal`, `alasan_banding`, `tanggal`, `pertanyaan_1`, `pertanyaan_2`, `pertanyaan_3`) VALUES ('$_GET[ida]', '$sk[id]', '$_GET[idj]', '$_POST[alasan_banding]', '$tglsekarang', '$_POST[pertanyaan_1]', '$_POST[pertanyaan_2]', '$_POST[pertanyaan_3]')";
					$conn->query($sqlinputak01);
				}
			// $sqlinputak02="UPDATE `asesi_asesmen` SET `umpan_balik`='".$_POST['umpan_balik']."', `tindak_lanjut_ak02`='".$_POST['tindaklanjut']."' WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]'";
			// $conn->query($sqlinputak02);
			...
			$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
			$alamatip = $_SERVER['REMOTE_ADDR'];
			$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.AK.04. FORMULIR BANDING ASESMEN','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
			$conn->query($sqlinputdigisign);	
				$sqlcekgetak01b="SELECT * FROM `asesmen_ak04` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' AND `id_skemakkni`='$sk[id]'";
				$cekgetak01b=$conn->query($sqlcekgetak01b);
				$jumak02unit=$cekgetak01b->num_rows;
			...
			<h4><i class='icon fa fa-check'></i> Formulir REKAMAN ASESMEN KOMPETENSI berhasil disimpan</h4>
			Terimakasih, Anda telah mengisi <b>Formulir REKAMAN ASESMEN KOMPETENSI untuk Skema $sk[judul].</b><br>
			<script>alert('Data berhasil disimpan'); window.location = '" . $base_url . "media.php?module=form-fr-ak-04&ida=$_SESSION[namauser]&idj=$_GET[idj]'</script></div>";
		}
	}
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.AK.04. FORMULIR BANDING ASESMEN' ORDER BY `waktu` DESC";
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();
	echo "<!-- Main content -->
              ...
              <!-- form start -->
            	<div class='box-body'>
					...
					<table id='example9' class='table table-bordered table-striped'>
						...
						<tr><td width='25%'>Nama Asesor</td><td colspan='2'>$namaasesor</td></tr>
						<tr><td width='25%'>Skema Sertifikasi (bila tersedia)</td><td colspan='2'>$sk[judul]</td></tr>
						<tr><td>Jawablah dengan Ya atau Tidak pertanyaan-pertanyaan berikut ini :</td><td>Ya</td><td>Tidak</td></tr>
						<tr><td>Apakah Proses Banding telah dijelaskan kepada Anda?</td><td><input type='radio' class='flat-red' name='pertanyaan_1' value='ya'";
						if($arrak04['pertanyaan_1'] == 'ya'){
							echo "checked";
						}else{
							echo "";
						}
						echo "></td><td><input type='radio' class='flat-red' name='pertanyaan_1' value='tidak'";
						if($arrak04['pertanyaan_1'] == 'tidak'){
							echo "checked";
						}else{
							echo "";
						}
						echo "></td></tr>
						<tr><td>Apakah Anda telah mendiskusikan Banding dengan Asesor?</td><td><input type='radio' class='flat-red' name='pertanyaan_2' value='ya'";
						if($arrak04['pertanyaan_2'] == 'ya'){
							echo "checked";
						}else{
							echo "";
						}
						echo "></td><td><input type='radio' class='flat-red' name='pertanyaan_2' value='tidak'";
						if($arrak04['pertanyaan_2'] == 'tidak'){
							echo "checked";
						}else{
							echo "";
						}
						echo "></td></tr>
						<tr><td>Apakah Anda mau melibatkan “orang lain” membantu Anda dalam Proses Banding?</td><td><input type='radio' class='flat-red' name='pertanyaan_3' value='ya'";
						if($arrak04['pertanyaan_3'] == 'ya'){
							echo "checked";
						}else{
							echo "";
						}
						echo "></td><td><input type='radio' class='flat-red' name='pertanyaan_3' value='tidak'";
						if($arrak04['pertanyaan_3'] == 'tidak'){
							echo "checked";
						}else{
							echo "";
						}
...
elseif ($_GET['module'] == 'form-ia-03') {
	...
	$noasr = 1;
	$getasesor = $conn->query("SELECT * FROM `jadwal_asesor` WHERE `id_jadwal`='$_GET[idj]'");
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.IA.03. PERTANYAAN UNTUK MENDUKUNG OBSERVASI' ORDER BY `waktu` DESC";
	// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();
	...
	if (isset($_REQUEST['simpan'])) {
		...
		if ($cekak01 > 0) {
			...
			if (empty($_POST['signed'])) {
				...
				<a class='btn btn-warning form-control' href='?module=jadwal'>Kembali</a></div>
				<script>window.location = '$base_url/media.php?module=form-ia-03&ida=$_GET[ida]&idj=$_GET[idj]'</script>";
			} else {
				$image_parts = explode(";base64,", $_POST['signed']);
				$image_type_aux = explode("image/", $image_parts[0]);
				$image_type = $image_type_aux[1];
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.IA.03. PERTANYAAN UNTUK MENDUKUNG OBSERVASI','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				// input tanggapan pendukung observasi
				$sqlgetunitkompetensib2 = "SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$sk[id]'";
				$getunitkompetensib2 = $conn->query($sqlgetunitkompetensib2);
			...
			if (empty($_POST['signed'])) {
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$_GET[id]'	,'$escaped_url','FR.IA.03. PERTANYAAN UNTUK MENDUKUNG OBSERVASI','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				// input tanggapan pendukung observasi
				$sqlgetunitkompetensib2 = "SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$sk[id]'";
				$getunitkompetensib2 = $conn->query($sqlgetunitkompetensib2);
	...
	if ($tgl_sekarang >= $jd['tgl_asesmen']) {
		...
		while ($unkb = $getunitkompetensib->fetch_assoc()) {
			...
			while ($gpp = $getpertanyaan->fetch_assoc()) {
				if (!empty($gpp['pertanyaan'])) {
					...
					if ($jumttd == 0) {
						echo "<textarea class='form-control' name='tanggapan$gpp[id]' readonly>";
		...
		if ($jumttd == 0) {
			echo "<textarea class='form-control' name='umpan_balik_ia03' disabled>";
...
elseif ($_GET['module'] == 'form-ia-04A') {
	$sqllogin = "SELECT * FROM `asesi` WHERE `no_pendaftaran`='$_SESSION[namauser]'";
	$login = $conn->query($sqllogin);
	$ketemu = $login->num_rows;
	$rowAgen = $login->fetch_assoc();
	...
	$noasr = 1;
	$getasesor = $conn->query("SELECT * FROM `jadwal_asesor` WHERE `id_jadwal`='$_GET[idj]'");
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.IA.04A. DIT - DAFTAR INSTRUKSI TERSTRUKTUR (PENJELASAN PROYEK SINGKAT/ KEGIATAN TERSTRUKTUR LAINNYA' ORDER BY `waktu` DESC";
	// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();
	while ($gas = $getasesor->fetch_assoc()) {
		...
		$sqlcektandatanganasesor = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$asr[no_ktp]' AND id_jadwal='$_GET[idj]' AND nama_dokumen='FR.IA.04A. DIT - DAFTAR INSTRUKSI TERSTRUKTUR (PENJELASAN PROYEK SINGKAT/ KEGIATAN TERSTRUKTUR LAINNYA' ORDER BY `waktu` DESC";
		$cektandatanganasesor = $conn->query($sqlcektandatanganasesor);
		$ttdasesor = $cektandatanganasesor->fetch_assoc();
		$noregasesor = $asr['no_induk'];
		$namaasesor = $noasr . '. ' . $namaasesor;
		$noregasesor = $noasr . '. ' . $noregasesor;
		$noasr++;
	...
	if (isset($_REQUEST['simpan'])) {
		...
		if ($cekak01 > 0) {
			...
			if (empty($_POST['signed'])) {
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`, `id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.IA.04A. DIT - DAFTAR INSTRUKSI TERSTRUKTUR (PENJELASAN PROYEK SINGKAT/ KEGIATAN TERSTRUKTUR LAINNYA','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				// input tanggapan pendukung observasi
				$sqlgetunitkompetensib2 = "SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$sk[id]'";
				$getunitkompetensib2 = $conn->query($sqlgetunitkompetensib2);
...
keluaran yang telah ditetapkan.</li>
		...
		<tr>
			<td colspan="4">
				<b>Umpan balik untuk Asesi :</b>
					<input class="form form-control" name="umpan_balik<?=$cta['id']?>" placeholder="Umpan Balik Untuk Asesi" value="<?=$gu['umpan_balik']?>" readonly></input>
			</td>
		</tr>
		<?php
		}
		...
		<table id='example9' class='table table-bordered table-striped'>
			...
			<tr>
				<td><img src="<?=$ttdx["file"] ?>" width="400px"/></td>
				<td><img src="<?= str_replace('../','',$ttdasesor["file"]) ?>" width="400px"/></td>
				<td></td>
			</tr>
		</table>
		<?php
...
// Bagian Input Hasil Asesmen FORM-FR.IA.04A
elseif ($_GET['module'] == 'form-ia-04B') {
	...
	$getskema = $conn->query($sqlgetskema);
	$sk = $getskema->fetch_assoc();
	$sqlgetasesordata = "SELECT * FROM `asesor` WHERE `id`='$jasr[id_asesor]'";
	$getasesordata = $conn->query($sqlgetasesordata);
	$asr1 = $getasesordata->fetch_assoc();
	$sqltuk = "SELECT * FROM `tuk` WHERE `id`='$jd[tempat_asesmen]'";
	$tuk = $conn->query($sqltuk);
	$tq = $tuk->fetch_assoc();
	$sqltukjenis = "SELECT `jenis_tuk` FROM `tuk_jenis` WHERE `id`='$tq[jenis_tuk]'";
	...
	// CEK TANDATANGAN ASESI
	$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.IA.04B. PENILAIAN PROYEK SINGKAT ATAU KEGIATAN TERSTRUKTUR LAINNYA' AND `id_jadwal`='$_GET[idj]' ORDER BY `waktu` DESC";
	$cektandatangan = $conn->query($sqlcektandatangan);
	$jumttd = $cektandatangan->num_rows;
	$ttdx = $cektandatangan->fetch_assoc();

	// CEK TANDATANGAN ASESOR
	$sqlcektandatanganasesor = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$asr1[no_ktp]' AND `penandatangan`='$asr1[nama]' AND nama_dokumen='FR.IA.04B. PENILAIAN PROYEK SINGKAT ATAU KEGIATAN TERSTRUKTUR LAINNYA' AND `id_jadwal`='$_GET[idj]' ORDER BY `waktu` DESC";
	$cekcektandatanganasesor = $conn->query($sqlcektandatanganasesor);
	$jumttasesor = $cekcektandatanganasesor->num_rows;
	$ttdasesor = $cekcektandatanganasesor->fetch_assoc();
	...
	// GET DATA ASESMEN FORM-FR-IA04B
		...
		if (!empty($asr1['gelar_depan'])) {
			if (!empty($asr1['gelar_blk'])) {
				$namaasesor = $asr1['gelar_depan'] . " " . $asr1['nama'] . ", " . $asr1['gelar_blk'];
			} else {
				$namaasesor = $asr1['gelar_depan'] . " " . $asr1['nama'];
			}
		} else {
			if (!empty($asr1['gelar_blk'])) {
				$namaasesor = $asr1['nama'] . ", " . $asr1['gelar_blk'];
			} else {
				$namaasesor = $asr1['nama'];
			}
		}
		$noregasesor = $asr1['no_induk'];
		$namaasesor = $noasr . '. ' . $namaasesor;
		$noregasesor = $noasr . '. ' . $noregasesor;
...
if (isset($_REQUEST['simpan'])) {
	...
	$tglsekarang = date("Y-m-d");
		...
		if (empty($_POST['signed'])) {
			...
			<a class='btn btn-warning form-control' href='?module=pesertaasesmen&idj=$_GET[idj]'>Kembali</a></div>"; ?>
							<script>
								window.location = "<?php echo $base_url ?>media.php?module=form-ia-04B&ida=<?php echo $_GET['ida'] ?>&idj=<?php echo $_GET['idj'] ?>"
							</script>
						<?php
		} else {
			$image_parts = explode(";base64,", $_POST['signed']);
...
presentasi</li>
				...
				$countfria04B=$conn->query("SELECT * FROM asesmen_ia04B a
					...
					$getlingkupkegiatanIA04B =$conn->query("SELECT * FROM lingkupkegiatan_formIA04B WHERE id_skemakkni='$sk[id]'");
					while ($gp = $getlingkupkegiatanIA04B->fetch_assoc()){
						$getfria04B=$conn->query("SELECT * FROM asesmen_ia04B a
						WHERE a.id_skemakkni=$jd[id_skemakkni] AND a.id_asesi='$_GET[ida]' AND a.id_jadwal='$_GET[idj]' AND a.id_pertanyaan='$gp[id]'");
						$gfr=$getfria04B->fetch_assoc();
							$getpertanyaanIA04B =$conn->query("SELECT * FROM skema_pertanyaania04B WHERE id_lingkupkegiatan='$gp[id]'");
							...
							echo "<tr>
									...
									<td></td>
									<td>"; 
									// while ($gc = $getpertanyaanIA04B->fetch_assoc()){
									?>
									<td>
										<div class="form-check">
											<input class="form-check-input" type="radio" name="pencapaian<?= $gp['id'] ?>" id="flexRadioDefault<?= $gp['id']?>" value="Ya" <?php if($gfr['pencapaian'] == 'Ya'){ echo "checked"; }?>>
										</div>
									</td>
									<td>
										<div class='form-check'>
											<input class="form-check-input" type="radio" name="pencapaian<?= $gp['id'] ?>" id="flexRadioDefault<?= $gp['id'] ?>" value="Tidak" <?php if($gfr['pencapaian'] == 'Tidak'){ echo "checked"; }?>>
										</div>
									</td>
							<?php
									// }
				...
				?>
							<div class="form-check">
								<input class="form-check-input" type="radio" name="rekomendasi" value="K" id="flexRadioDefault1" <?php if($gtas['rekomendasi_IA04B'] == 'K'){ echo 'checked'; }else{ echo "disabled"; }?>>
								<label class="form-check-label" for="flexRadioDefault1">
									Kompeten
								</label>
							</div>		
							<div class="form-check">
								<input class="form-check-input" type="radio" name="rekomendasi" value="BK" id="flexRadioDefault2" <?php if($gtas['rekomendasi_IA04B'] == 'BK'){ echo 'checked'; }else{ echo "disabled"; }?>>
								<label class="form-check-label" for="flexRadioDefault1">
									Belum Kompeten
								</label>
							</div>
				<?php		
					...
					$getk = $getkeputusan->fetch_assoc();
					if ($jumttd > 0) { ?>
						<td><?php echo $tanggalttdasesor; ?><br><img src='<?= str_replace('../','',$ttdasesor['file']) ?>' width='400px'/></td>
					<?php echo "</tr>
				</table>";
			} else {
				echo "
...
elseif ($_GET['module'] == 'form-ia-05') {
	...
	// cek triger asesor
	$sqlgettrigersoal = "SELECT * FROM `asesi_aksessoal` WHERE `id_asesi`='$_SESSION[namauser]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]' AND `jenis_soal`='FR.IA.05'";
	$gettrigersoal = $conn->query($sqlgettrigersoal);
	$jumtriger = $gettrigersoal->num_rows;
	$arrsoal = $gettrigersoal->fetch_assoc();
	if ($arrsoal['status'] == 1) {
		echo "<!-- Main content -->
		<section class='content'>
		  <div class='row'>
		  ...
		  <div class='row'>
			<div class='col-xs-12'>
			  <div class='box'>
				...
				<div class='box-body box-success'>
				<div class='callout callout-warning'>
                    <h4>Selamat, Anda Sudah Melakukan Tes</h4>
...
elseif ($_GET['module'] == 'form-ia-06') {
			...
			if (empty($_POST['signed'])) {
				...
				while ($unkb2 = $getunitkompetensib2->fetch_assoc()) {
					...
					while ($gpp2 = $getpertanyaan2->fetch_assoc()) {
						...
						$conn->query($sqlinputak01);
						$updateia06 = "UPDATE `asesi_aksessoal` SET `status` = '2' WHERE id_asesi ='$_SESSION[namauser]' AND id_skemakkni=$jd[id_skemakkni] AND `id_jadwal`='$_GET[idj]' AND jenis_soal='FR.IA.06'";
						$conn->query($updateia06);
						//}
					}
				}
				...
				$escaped_url = htmlspecialchars($url, ENT_QUOTES, 'UTF-8');
				$alamatip = $_SERVER['REMOTE_ADDR'];
				$sqlinputdigisign = "INSERT INTO `logdigisign`(`id_dokumen`, `id_asesi`, `id_skema`, `url_ditandatangani`, `nama_dokumen`, `penandatangan`, `file`, `ip`,`id_jadwal`) VALUES ('$iddokumen','$_SESSION[namauser]','$jd[id_skemakkni]','$escaped_url','FR.IA.06.PERTANYAAN TERTULIS ESAI','$rowAgen[nama]','$file','$alamatip','$_GET[idj]')";
				$conn->query($sqlinputdigisign);
				// input jawaban pendukung observasi
				$sqlgetunitkompetensib2 = "SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$sk[id]'";
				$getunitkompetensib2 = $conn->query($sqlgetunitkompetensib2);
	...
	$tanggalasesmen = tgl_indo($jd['tgl_asesmen']);
	// cek triger asesor
	$sqlgettrigersoal = "SELECT * FROM `asesi_aksessoal` WHERE `id_asesi`='$_SESSION[namauser]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]' AND `jenis_soal`='FR.IA.06'";
	$gettrigersoal = $conn->query($sqlgettrigersoal);
	$jumtriger = $gettrigersoal->num_rows;
	$cekdataai06 = $gettrigersoal->fetch_assoc();
	if ($cekdataai06['status'] == 1) {
		echo "<!-- Main content -->
		<section class='content'>
		  <div class='row'>
			<div class='col-xs-12'>
			...
			while ($gpp = $getpertanyaan->fetch_assoc()) {
				if (!empty($gpp['pertanyaan'])) {
					...
					echo " disabled>&nbsp;Belum Kompeten</td></tr>
									...
									<textarea class='form-control' name='jawaban$gpp[id]' required>";
		...
		// $sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE `id_dokumen`='$iddokumen' AND `penandatangan`='$_SESSION[namalengkap]' ORDER BY `id` DESC";
		// UPDATE @FHM-PUSTI 1 AGUSTUS 2023
		$sqlcektandatangan = "SELECT * FROM `logdigisign` WHERE id_skema='$jd[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' AND `penandatangan`='$rowAgen[nama]' AND nama_dokumen='FR.IA.06.PERTANYAAN TERTULIS ESAI' ORDER BY `waktu` DESC";
		$cektandatangan = $conn->query($sqlcektandatangan);
		$jumttd = $cektandatangan->num_rows;
		$ttdx = $cektandatangan->fetch_assoc();
		if ($jumttd > 0) {
		...
		while ($unkb = $getunitkompetensib->fetch_assoc()) {
			...
			while ($gpp = $getpertanyaan->fetch_assoc()) {
				if (!empty($gpp['pertanyaan'])) {
					...
					echo " disabled>&nbsp;Belum Kompeten</td></tr>
									<tr><td colspan='2'>Jawaban:<br>
									<textarea class='form-control' name='jawaban$gpp[id]' readonly>";
					if (!empty($jjw['jawaban'])) {
						echo $jjw['jawaban'];
					} else {
						echo "";
