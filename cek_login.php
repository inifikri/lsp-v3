...
$sqllogin="SELECT * FROM `asesi` WHERE `no_pendaftaran`='$username' AND `password`='$pass' AND `blokir`='N' OR `no_ktp`='$username' AND `password`='$pass' AND `blokir`='N'";
...
if ($ketemu > 0){
  session_start();
	...
	$logout = "asesi/index.php"; // redirect halaman logout
  ...
  $_SESSION['tgl_lahir']    = $r['tgl_lahir'];
...
//mysql_query("INSERT INTO `loguser`(`username`, `waktu`, `IP`, `from`,`inorout`) VALUES ('$r[username]','$tgljam','$ip','http://admin.ivet.ac.id','1')");

	if ($r['tgl_lahir']=='0000-00-00'){
		header('location:media.php?module=editprofil');
	}else{
...
$sqllogin2="SELECT * FROM `users` WHERE `username`='$_POST[username]'";
...
<div class='lockscreen-wrapper'>
  <div class='lockscreen-logo'>
    <a href='index.php'><b>Dashboard</b>Asesi</a>
  </div>
  <!-- User name -->
  ...
  <div class='lockscreen-item'>
    ...
    <div class='lockscreen-image'>";
      	if(empty($rf['foto'])){
		echo"<img src='images/default.jpg' alt='User Image'>";
	}else{
		echo"<img src='../foto_asesi/$rf[foto]' alt='User Image'>";
	}
    echo"</div>
    <!-- /.lockscreen-image -->
