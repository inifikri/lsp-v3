...
<div class="login-box">
  <div class="login-logo">
   Reset Password Login Komite Teknis
  </div>
  <!-- /.login-logo -->
  ...
  <br>
	...
	if( isset( $_REQUEST['reset'] )){
		...
		if ($kodeaman!=$kodeaman2){
			...
			$periksa=$conn->query("SELECT * FROM `komite` WHERE `no_induk`='$npm' AND `no_induk`!=''") ;
			switch ($pertanyaan){
				default :
				...
				case 'nomorktp':
					...
					$smsnya = "Yth. $ds1[nama], Kata sandi baru Anda adalah $generatepassword Silahkan ganti password Anda di Laman http://".$_SERVER['HTTP_HOST']."/komite";

					if ($jawaban==$ds1[no_ktp]){
						echo "<center>Yth. <b>$ds1[nama]</b>, Kata sandi baru Anda adalah:<br /><font color='red' size='5'><b>$generatepassword</b></font><br />Silahkan ganti password Anda di Laman Komite Teknis<br><a href='index.php' class='btn btn-primary btn-block btn-flat'>Masuk Laman Komite Teknis</a></center>";
						$sql2 = "UPDATE `komite` SET `password`='$password' WHERE `no_induk`='$npm' AND `no_induk`!=''";
				...
				case 'tahunlahir':
					$ds1 = $periksa->fetch_assoc();
					$smsnya = "Yth. $ds[nama], Kata sandi baru Anda adalah $generatepassword Silahkan ganti password Anda di Laman http://".$_SERVER['HTTP_HOST']."/komite";
...
$dsn=$conn->query("SELECT `id`, `nama`, `gelar_depan`, `gelar_blk` FROM `komite` ORDER BY `nama` ASC");
?>       </div>
      <div class="form-group has-feedback">
        <input required type="text" name="npm" class="form-control" placeholder="No. Induk Komite Teknis">
      </div>
<div class="form-group has-feedback">
