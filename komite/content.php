...
if ($_GET['module']=='home'){
	if (!empty($_SESSION['namauser'])){
		$sqljumkomite0="SELECT * FROM `komite` WHERE `no_ktp`='$_SESSION[namauser]'";
		$qjumkomite0=$conn->query($sqljumkomite0);
		$asr0=$qjumkomite0->fetch_assoc();
		$tahun=date("Y");
		//$jumtuk=0;
		//Jumlah Skema Sertifikasi
		$sqljumskema="SELECT DISTINCT `id_skemakkni` FROM `komite_keputusan` WHERE `id_komite`='$asr0[id]'";
		$qjumskema=$conn->query($sqljumskema);
		$jumskema=$qjumskema->num_rows;
		//Jumlah TUK Sertifikasi
		$sqljumtuk="SELECT DISTINCT `id_jadwal` FROM `komite_keputusan`";
		$qjumtuk=$conn->query($sqljumtuk);
		$jumtuk=$qjumtuk->num_rows;
		//Jumlah Portofolio komite Sertifikasi
		$sqljumkomite="SELECT DISTINCT `id_asesi` FROM `komite_keputusan` WHERE `id_komite`='$asr0[id]'";
		$qjumkomite=$conn->query($sqljumkomite);
		$jumkomite=$qjumkomite->num_rows;
		//Jumlah komite Sertifikasi
		$sqljumasesi="SELECT DISTINCT `id_asesi` FROM `komite_keputusan` WHERE `id_komite`='$asr0[id]' AND `waktu` LIKE '$tahun%'";
...
$sqllogin="SELECT * FROM `komite` WHERE `no_ktp`='$_SESSION[namauser]'";
...
$rowAgen=$login->fetch_assoc();
						if (!empty($rowAgen['gelar_depan'])){
							if (!empty($rowAgen['gelar_blk'])){
								$namakomite=$rowAgen['gelar_depan']." ".$rowAgen['nama'].", ".$rowAgen['gelar_blk'];
							}else{
								$namakomite=$rowAgen['gelar_depan']." ".$rowAgen['nama'];
							}
						}else{
							if (!empty($rowAgen['gelar_blk'])){
								$namakomite=$rowAgen['nama'].", ".$rowAgen['gelar_blk'];
							}else{
								$namakomite=$rowAgen['nama'];
							}
						}
...
echo "<!-- Content Header (Page header) -->
    <section class='content-header'>
      <h1>
	  Profil Komite Teknis
        <small></small>
      </h1>
      <ol class='breadcrumb'>
        <li><a href='media.php?module=home'><i class='fa fa-dashboard'></i> Home</a></li>
        <li class='active'>Profil Komite Teknis</li>
      </ol>
    </section>
    <!-- Main content -->
    <section class='content'>
      ...
      <div class='row'>
		...
		if (empty($rowAgen['foto'])){
              		...
              		echo"<img class='profile-user-img img-responsive img-circle' src='../foto_komite/$rowAgen[foto]' alt='User profile picture' style='width:128px; height:128px;'>";
		}

              echo "<h3 class='profile-username text-center'>$namakomite</h3>

              <p class='text-muted text-center'>$rowAgen[no_ktp]</p>
			  <br>
			  <a href='?module=updatekomite' class='btn btn-block btn-primary'>Ubah Profil</a>
			  <br>
		...
		<!-- About Me Box -->
          <div class='box box-primary'>
            <div class='box-header with-border'>
              <strong><h3 class='box-title'>Detail Data Komite Teknis</h3></strong>
            </div>
            <!-- /.box-header -->
            <div class='box-body'>
			<br>
              <strong class='col-md-3'>Nomor Induk Komite Teknis</strong>
              ...
              <strong class='col-md-3'>Nama Lengkap dan Gelar</strong>
               <span class='col-md-9 col-sm-12 col-xs-12'>$namakomite</span>
			   <br><br>
...
if( isset( $_REQUEST['hapusjadwal'] ))
    ...
    <section class='content-header'>
      <h1>
        ...
        <small>Pengambilan Keputusan Tim Komite Teknis</small>
      </h1>
      <ol class='breadcrumb'>
        <li><a href='media.php?module=home'><i class='fa fa-dashboard'></i> Home</a></li>
        <li class='active'>Jadwal Uji Kompetensi</li>
      </ol>
    </section>
    ...
    <section class='content'>
      <div class='row'>
        <div class='col-xs-12'>
          <div class='box'>
            <div class='box-header'>
              <h3 class='box-title'>Data Jadwal Uji Kompetensi Lembaga Sertifikasi Profesi dimana Anda sebagai Tim Komite Teknis</h3>
            </div>
            <!-- /.box-header -->
            <div class='box-body'>
			<div style='overflow-x:auto;'>
				<table id='example1' class='table table-bordered table-striped'>
					...
					<tbody>";
					$no=1;
					$sqljadwaltukx="SELECT * FROM `jadwal_asesmen` WHERE `voting_komite` IS NULL ORDER BY `tgl_asesmen` DESC";
							...
							<button type='button' class='btn btn-success dropdown-toggle' data-toggle='dropdown'>
							  <span class='caret'></span>
                  					...
                  					<ul class='dropdown-menu' role='menu'>
                  					  ...
                  					  <li><a href='../asesor/daftarhadir.php?idj=$pm[id]' title='Unduh Berita Acara dan Daftar Hadir Peserta'>Berita Acara & Daftar Hadir</a></li>
                  					  <li class='divider'>Unduh MAPA 1</li>
                  					  <li><a href='../asesor/form-mapa-01.php?idsk=$pm[id_skemakkni]&kand=1' title='Unduh MAPA 01-MERENCANAKAN AKTIVITAS DAN PROSES ASESMEN'>Unduh Form MAPA-01 Kandidat 1</a></li>
                  					  <li><a href='../asesor/form-mapa-01.php?idsk=$pm[id_skemakkni]&kand=2' title='Unduh MAPA 01-MERENCANAKAN AKTIVITAS DAN PROSES ASESMEN'>Unduh Form MAPA-01 Kandidat 2</a></li>
                  					  <li><a href='../asesor/form-mapa-01.php?idsk=$pm[id_skemakkni]&kand=3' title='Unduh MAPA 01-MERENCANAKAN AKTIVITAS DAN PROSES ASESMEN'>Unduh Form MAPA-01 Kandidat 3</a></li>
                  					  <li><a href='../asesor/form-mapa-01.php?idsk=$pm[id_skemakkni]&kand=4' title='Unduh MAPA 01-MERENCANAKAN AKTIVITAS DAN PROSES ASESMEN'>Unduh Form MAPA-01 Kandidat 4</a></li>
                  					  <li><a href='../asesor/form-mapa-01.php?idsk=$pm[id_skemakkni]&kand=5' title='Unduh MAPA 01-MERENCANAKAN AKTIVITAS DAN PROSES ASESMEN'>Unduh Form MAPA-01 Kandidat 5</a></li>
                  					  <li class='divider'></li>
                  					  <li><a href='../asesor/form-mapa-02.php?idsk=$pm[id_skemakkni]' title='Unduh MAPA 02-PETA INSTRUMEN ASESMEN HASIL PENDEKATAN ASESMEN DAN PERENCANAAN ASESMEN'>Unduh Form MAPA-02</a></li>
							  <li><a href='../asesor/form-ak-05.php?idj=$pm[id]' title='Unduh AK 05-LAPORAN ASESMEN'>Unduh Form AK-05</a></li>
							  <li><a href='../asesor/form-ak-06.php?idj=$pm[id]' title='Unduh AK 06-MENINJAU PROSES ASESMEN'>Unduh Form AK-06</a></li>
  ...
  if (!empty($_SESSION['namauser'])){
	...
	{	
		...
		if(empty($_POST['signed'])){
			echo "<div class='alert alert-success alert-dismissible'>
			<button type='button' class='close' data-dismiss='alert' aria-hidden='true'>&times;</button>
			<h4><i class='icon fa fa-check'></i> Persetujuan Berita Acara berhasil disimpan</h4>
			Terimakasih, Anda telah melakukan <b>Persetujuan Berita Acara Pleno Komite Teknis untuk Skema $sk[judul].</b><br>
			<a class='btn btn-warning form-control' href='?module=jadwalasesmen&idj=$_GET[idj]'>Kembali</a></div>";
		}else{
			$image_parts = explode(";base64,", $_POST['signed']); 
			$image_type_aux = explode("image/", $image_parts[0]);
			$image_type = $image_type_aux[1];
        ...
        <div class='col-xs-12'>
			...
			<div style='overflow-x:auto;'>
				<table id='example1' class='table table-bordered table-striped'>
					...
					while ($pm=$jadwaltuk->fetch_assoc()){
						...
						echo "<div class='btn-group'>
							...
							<button type='button' class='btn btn-success dropdown-toggle' data-toggle='dropdown'>
							  <span class='caret'></span>
                  					...
                  					<ul class='dropdown-menu' role='menu'>
                  					  <li><a href='../asesor/form-checklist.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR CHECKLIST'>Unduh Checklist Form</a></li>
                  					  <!--<li><a href='form-mak01.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR CHECKLIST MENGASES KOMPETENSI'>Unduh FR-MAK-01</a></li>-->
                  					  <li><a href='../asesor/form-apl-01.php?ida=$pm[id_asesi]&idj=$_GET[idj]'>Unduh APL-01</a></li>
                  					  <li><a href='../asesor/portfolio-asesi.php?ida=$pm[id_asesi]&idj=$_GET[idj]'>Unduh Portfolio Asesi</a></li>
                  					  <li>";
										...
										$cekjenisapl02="SELECT `apl02` FROM `skema_kkni` WHERE `id`='$pm[id_skemakkni]'";
										$jenisapl02=$conn->query($cekjenisapl02);
										$jnsapl02=$jenisapl02->fetch_assoc();
									  echo"</li>";
									  if ($jnsapl02['apl02']=="elemen"){
										echo "<li><a href='../asesor/form-apl-02-el.php?ida=$pm[id_asesi]&idj=$_GET[idj]'>Unduh Formulir APL-02</a></li>";
									  }else{
										echo "<li><a href='../asesor/form-apl-02.php?ida=$pm[id_asesi]&idj=$_GET[idj]'>Unduh Formulir APL-02</a></li>";
                  					  }
									  echo "<li><a href='../asesor/form-mapa-01.php?idsk=$pm[id_skemakkni]' title='FORMULIR MERENCANAKAN AKTIVITAS DAN PROSES ASESMEN'>Unduh FORM-MAPA-01</a></li>
                  					  <li><a href='../asesor/form-mapa-02.php?idsk=$pm[id_skemakkni]' title='FORMULIR PETA MUK DARI HASIL PENDEKATAN ASESMEN DAN PERENCANAAN ASESMEN'>Unduh FORM-MAPA-02</a></li>
                  					  <li><a href='../asesor/form-fr-ak-01.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR PERSETUJUAN ASESMEN DAN KERAHASIAAN'>Unduh Formulir FR-AK-01</a></li>
                  					  <li><a href='../asesor/form-fr-ak-02.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR REKAMAN ASESMEN KOMPETENSI'>Unduh Formulir FR-AK-02</a></li>
                  					  <li><a href='../asesor/form-fr-ak-03.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR UMPAN BALIK DAN CATATAN ASESMEN'>Unduh Formulir FR-AK-03</a></li>
                  					  <li><a href='../asesor/form-fr-ak-04.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR BANDING ASESMEN'>Unduh Formulir FR-AK-04</a></li>
                  					  <li><a href='../asesor/form-fr-ia-01.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='FORMULIR CEKLIS OBSERVASI AKTIVITAS DI TEMPAT KERJA ATAU TEMPAT KERJA SIMULASI'>Unduh Formulir FR-IA-01</a></li>
                  					  <li><a href='../asesor/form-fr-ia-02.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='TUGAS PRAKTIK DEMONSTRASI'>Unduh Formulir FR-IA-02</a></li>
                  					  <li><a href='../asesor/form-fr-ia-03.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='PERTANYAAN UNTUK MENDUKUNG OBSERVASI'>Unduh Formulir FR-IA-03</a></li>
                  					  <li><a href='../asesor/form-fr-ia-05.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='PERTANYAAN TERTULIS PILIHAN GANDA'>Unduh Formulir FR-IA-05</a></li>
                  					  <li><a href='../asesor/form-fr-ia-05b.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='JAWABAN PERTANYAAN TERTULIS PILIHAN GANDA'>Unduh Jawaban Asesi FR-IA-05</a></li>
                  					  <li><a href='../asesor/form-fr-ia-06.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='PERTANYAAN TERTULIS ESAI'>Unduh Formulir FR-IA-06</a></li>
                  					  <li><a href='../asesor/form-fr-ia-06a.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='LEMBAR KUNCI JAWABAN PERTANYAAN TERTULIS ESAI'>Unduh Formulir FR-IA-06.A</a></li>
                  					  <li><a href='../asesor/form-fr-ia-06b.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='LEMBAR JAWABAN PERTANYAAN TERTULIS ESAI'>Unduh Formulir FR-IA-06.B</a></li>
                  					  <li><a href='../asesor/form-fr-ia-07.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='PERTANYAAN LISAN'>Unduh Formulir FR-IA-07</a></li>
                  					  <li><a href='../asesor/form-fr-ia-08.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='CEKLIS VERIFIKASI PORTOFOLIO'>Unduh Formulir FR-IA-08</a></li>
                  					  <li><a href='../asesor/form-fr-ia-10.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='KLARIFIKASI BUKTI PIHAK KETIGA'>Unduh Formulir FR-IA-10</a></li>
                  					  <li><a href='../asesor/form-fr-ia-11.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='CEKLIS MENINJAU INSTRUMEN ASESSMEN'>Unduh Formulir FR-IA-11</a></li>
                  					  <li><a href='../asesor/form-fr-va.php?ida=$pm[id_asesi]&idj=$_GET[idj]' title='MEMBERIKAN KONTRIBUSI DALAM VALIDASI ASESMEN'>Unduh Formulir FR-VA</a></li>
                  					</ul>
                				</div>";
						$cekstatusAPL02="SELECT `id_asesi` FROM `asesi_apl02` WHERE `id_asesi`='$pm[id_asesi]' AND `id_jadwal`='$_GET[idj]'";
						$statusapl02=$conn->query($cekstatusAPL02);
						$jumapl02=$statusapl02->num_rows;
						$cekstatusasesmen="SELECT * FROM `komite_keputusan` WHERE `id_asesi`='$pm[id_asesi]' AND `id_jadwal`='$_GET[idj]' AND `id_komite`='$_SESSION[id]'";
						$statusasesmen=$conn->query($cekstatusasesmen);
						$gstass=$statusasesmen->fetch_assoc();
						$jumasesmen=$statusasesmen->num_rows;
						echo "</td><td><b>$as[nama]</b><br>No. Pendaftaran : $pm[id_asesi]<br>";
						if (!empty($as['nohp'])){
								/* echo "<form name='frm' method='POST' role='form' enctype='multipart/form-data'><a href='#myModalSMS".$as['no_pendaftaran']."' data-toggle='modal' data-id='".$as['no_pendaftaran']."' class='btn btn-xs btn-primary' title='Kirim pesan SMS ke $as[nama] ($as[nohp])'><i class='fa fa-envelope'></i> Kirim SMS</a>
								<input type='hidden' name='nohpasesi' value='$as[nohp]'>
								<input type='hidden' name='namaasesi' value='$as[nama]'>
						...
						if ($jumapl02>0){echo "<br><font color='red'>Telah mengisi Asesmen Mandiri (APL-02)</font>";}
						if ($jumasesmen>0){echo "<br><font color='green'><b>Keputusan telah dibuat dan Anda sebagai Komite Teknis menyatakan</b></font> : ";
							switch ($gstass['keputusan']){
								case "K":
									echo "<font color='green'><b>KOMPETEN</b></font>";
								break;
								case "BK":
						...
						if ($jumasesmen>0){
							echo "<form role='form' action='keputusankomite.php' method='POST' enctype='multipart/form-data'>
							...
							<input type='hidden' name='id_komite' value='$_SESSION[id]'>
							<input type='hidden' name='id_skemakkni' value='$pm[id_skemakkni]'>
							<div class='btn-group'>
							<button type='button' class='btn btn-primary btn-xs'>Keputusan (Komite Teknis)</button>
							<button type='button' class='btn btn-primary btn-xs dropdown-toggle' data-toggle='dropdown'>
							  <span class='caret'></span>
                  					  <span class='sr-only'>Toggle Dropdown</span>
                  					</button>
                  					<ul class='dropdown-menu' role='menu'>
									<li><input type='submit' name='keputusanK' class='btn btn-success btn-flat btn-block' value='Putuskan Kompeten (K)'></li>
									<li><input type='submit' name='keputusanBK' class='btn btn-danger btn-flat btn-block' value='Putuskan Belum Kompeten (BK)'></li>
									<li><input type='submit' name='keputusanTL' class='btn btn-warning btn-flat btn-block' value='Putuskan Uji Ulang/ Tindak Lanjut (TL)'></li>
		  ...
		  <div class='box'>
			...
			<form role='form' method='POST' enctype='multipart/form-data'>";
				...
				echo "<div class='col-md-12'>
					<label class='' for=''>Tanda Tangan Komite :</label>
					<br/>
					<div id='sig' ></div>
					<br/>
					<button id='clear'>Hapus Tanda Tangan</button>
			...
			<div class='box-footer'>
				<div align='left' class='col-md-6 col-sm-6 col-xs-6'>";
					echo "<input type='submit' name='simpanpleno' value='Simpan' class='btn btn-success'>";
				echo"</div>
				<div align='right' class='col-md-6 col-sm-6 col-xs-6'>";
					echo "<a href='?module=jadwalasesmen' class='btn btn-primary'>Lihat Jadwal Lainnya</a>";
				echo"</div>
			</div>
			</form>
	...
	{				
		...
		if ($ukom>0){
			...
			while ($el2=$getelemen2->fetch_assoc()){
				...
				$jjw=$cekjawaban->num_rows;
				if ($jjw==0){
					$sqlinputjawaban="INSERT INTO `asesi_apl02`(`id_asesi`, `id_elemen`, `id_skemakkni`, `jawaban`, ``, ``) VALUES ('$_GET[ida]','$el2[id]','$sk[id]','$_POST[$id_jawaban]','$_POST[$id_jawabanb]','$_POST[$id_jawabanc]')";
					$conn->query($sqlinputjawaban);
				}else{
					$sqlinputjawaban="UPDATE `asesi_apl02` SET `keterangan_bukti`='$_POST[$id_jawaban0]',`verifikasi_asesor1`='$_POST[$id_jawaban]',`verifikasi_asesor2`='$_POST[$id_jawabanb]',`verifikasi_asesor3`='$_POST[$id_jawabanc]',`verifikasi_asesor4`='$_POST[$id_jawaband]',`id_asesor`='$asr[id]' WHERE `id_asesi`='$_GET[ida]' AND `id_elemen`='$el2[id]' AND `id_skemakkni`='$sk[id]' AND `id_jadwal`='$_GET[idj]'";
					$conn->query($sqlinputjawaban);
	...
	{				
		...
		if ($cekak01>0){
			...
			if(empty($_POST['signed'])){
				...
				<a class='btn btn-warning form-control' href='?module=pesertaasesmen&idj=$_GET[idj]'>Kembali</a></div>";
				$sqlinputak01="UPDATE `asesmen_ak01` SET `VP`='$_POST[checkboxVP]',`CL`='$_POST[checkboxCL]',`DPT`='$_POST[checkboxDPT]',`DPL`='$_POST[checkboxDPL]',`PW`='$_POST[checkboxPW]',`persetujuan`='$_POST[persetujuan]',`tanggal`='$tglsekarang' WHERE `id_asesi`='$_GET[ida]' AND `id_skemakkni`='$jd[id_skemakkni]' AND `id_jadwal`='$_GET[idj]'";
			...
			if(empty($_POST['signed'])){
				...
				Terimakasih, Anda telah melakukan <b>Persetujuan Asesmen dan Kerahasiaan untuk Skema $sk[judul].</b><br>
				<a class='btn btn-warning form-control' href='?module=pesertaasesmen&idj=$_GET[idj]'>Kembali</a></div>";
				$sqlinputak01="INSERT INTO `asesmen_ak01`(`id_asesi`, `id_skemakkni`, `id_jadwal`, `VP`, `CL`, `DPT`, `DPL`, `PW`, `persetujuan`, `tanggal`) VALUES ('$_GET[ida]','$jd[id_skemakkni]','$_GET[idj]','$_POST[checkboxVP]','$_POST[checkboxCL]','$_POST[checkboxDPT]','$_POST[checkboxDPL]','$_POST[checkboxPW]','$_POST[persetujuan]','$tglsekarang')";
				$conn->query($sqlinputak01);
			}else{
				$image_parts = explode(";base64,", $_POST['signed']); 
				$image_type_aux = explode("image/", $image_parts[0]);
...
echo "<!-- Content Header (Page header) -->
	...
	if( isset( $_REQUEST['simpan'] ))
	{				
		$sqlcekgetak01="SELECT * FROM `asesmen_ak05` WHERE `id_asesor`='$isr[id]' AND `id_jadwal`='$_GET[idj]'";
		$cekgetak01=$conn->query($sqlcekgetak01);
		$cekak01=$cekgetak01->num_rows;
		$tglsekarang=date("Y-m-d");
				...
				while ($unkb2=$getunitkompetensib2->fetch_assoc()){
					...
					while ($gpp2=$getpertanyaan2->fetch_assoc()){
						//if (!empty($gpp2['pertanyaan'])){
							...
							$sqlinputak01="UPDATE `asesmen_ia03` SET `tanggapan`='".$_POST[$posttanggapan]."', `rekomendasi`='".$_POST[$postrekomendasi]."' WHERE `id_asesi`='$_GET[ida]' AND `id_skemakkni`='$jd[id_skemakkni]' AND `id_jadwal`='$_GET[idj]'";
							$conn->query($sqlinputak01);
	...
	{				
		...
		if ($cekak01>0){
			...
			if(empty($_POST['signed'])){
				...
				//$getunitkompetensib2=$conn->query($sqlgetunitkompetensib2);
				//while ($unkb2=$getunitkompetensib2->fetch_assoc()){
					$sqlgetpertanyaan2="SELECT * FROM `asesi_portfolio` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' ORDER BY `id` ASC";
					$getpertanyaan2=$conn->query($sqlgetpertanyaan2);
					while ($gpp2=$getpertanyaan2->fetch_assoc()){
						//if (!empty($gpp2['pertanyaan'])){
			...
			<div class='box-body'>
			  <!-- form start -->
				<div class='box-body'>
					...
					<tbody>";
					// dokumen portfolio
					$sqlgetunitkompetensib="SELECT * FROM `asesi_portfolio` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' ORDER BY `id` ASC";
					$getunitkompetensib=$conn->query($sqlgetunitkompetensib);
					$nopp=1;
					while ($unkb=$getunitkompetensib->fetch_assoc()){
						...
						$cekjawaban=$conn->query($sqlcekjawaban);
						$jjw=$cekjawaban->fetch_assoc();
						echo "<tr><td>$nopp</td><td>".$unkb['jenis_portfolio']."<br>".$unkb['nama_doc']."<br>".$unkb['peran_portfolio']."<br>Tanggal Dokumen: ".tgl_indo($unkb['tgl_doc'])."<br><a href='../foto_portfolio/".$unkb['file']."' target='_blank' class='btn btn-primary btn-xs'>Lihat Dokumen</a></td>
					...
					<tbody>";
					$sqlgetunitkompetensib2="SELECT * FROM `asesi_portfolio` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' ORDER BY `id` ASC";
					$getunitkompetensib2=$conn->query($sqlgetunitkompetensib2);
					$nopp2=1;
					while ($unkb2=$getunitkompetensib2->fetch_assoc()){
...
elseif ($_GET['module']=='form-fr-ia-09'){
	...
	{				
		...
		// input data ia09 asesor
		$sqlcekgetak01a="SELECT * FROM `asesmen_ia09asesor` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' AND `id_asesor`='$_SESSION[namauser]'";
		$cekgetak01a=$conn->query($sqlcekgetak01a);
		$cekak01a=$cekgetak01a->num_rows;
		if ($cekak01a>0){
			$sqlinputia09asesor="UPDATE `asesmen_ia09asesor` SET `id_asesi`='$_GET[ida]',`id_jadwal`='$_GET[idj]',`id_asesor`='$_SESSION[namauser]',`buktitambahan`='$_POST[buktitambahan]',`contohbukti`='$_POST[contohbukti]',`rekomendasi`='$_POST[rekomendasi]',`unitbkrekom`='$_POST[unitbkrekom]',`elemenbkrekom`='$_POST[elemenbkrekom]',`kukbkrekom`='$_POST[kukbkrekom]' WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' AND `id_asesor`='$_SESSION[namauser]'";
			$conn->query($sqlinputia09asesor);
		}else{
			$sqlinputia09asesor="INSERT INTO `asesmen_ia09asesor`(`id_asesi`, `id_jadwal`, `id_asesor`, `buktitambahan`, `contohbukti`, `rekomendasi`, `unitbkrekom`, `elemenbkrekom`, `kukbkrekom`) VALUES ('$_GET[ida]','$_GET[idj]','$_SESSION[namauser]','$_POST[buktitambahan]','$_POST[contohbukti]','$_POST[rekomendasi]','$_POST[unitbkrekom]','$_POST[elemenbkrekom]','$_POST[kukbkrekom]')";
			$conn->query($sqlinputia09asesor);
		}
	}
			...
			<div class='box-body'>
			  <!-- form start -->
				<div class='box-body'>
					...
					$nopp=1;
					while ($unkb=$getunitkompetensib->fetch_assoc()){
						$sqlcekjawaban="SELECT * FROM `asesmen_ia09` WHERE `id_asesi`='$_GET[ida]' AND `id_jadwal`='$_GET[idj]' AND `id_portfolio`='$unkb[id]'";
						$cekjawaban=$conn->query($sqlcekjawaban);
						$jjw=$cekjawaban->fetch_assoc();
						echo "<tr><td>$nopp</td><td>".$unkb['jenis_portfolio']."<br>".$unkb['nama_doc']."<br>".$unkb['peran_portfolio']."<br>Tanggal Dokumen: ".tgl_indo($unkb['tgl_doc'])."<br><a href='../foto_portfolio/".$unkb['file']."' target='_blank' class='btn btn-primary btn-xs'>Lihat Dokumen</a></td>
						</tr>";
...
elseif ($_GET['module']=='mapa1a'){
    ...
    <section class='content-header'>
      ...
      <ol class='breadcrumb'>
        <li><a href='media.php?module=home'><i class='fa fa-dashboard'></i> Home</a></li>
        <li><a href='media.php?module=skema'></i> Skema Sertifikasi</a></li>
        <li class='active'>Data Instrumen Skema Sertifikasi Profesi</li>
      </ol>
    </section>
      ...
      <div class='row'>
        <div class='col-xs-12'>
          <div class='box'>
			...
			<div class='box-footer'>
				<div class='col-md-4 col-sm-12 col-xs-12'>
					<a class='btn btn-danger form-control' id=reset-validate-form href='?module=skema'>Kembali</a>
				</div>
				<div class='col-md-4 col-sm-12 col-xs-12'>
...
elseif ($_GET['module']=='mapa1b'){
	if( isset( $_REQUEST['simpanbagian'] )){
		$sqlgetunit2="SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$_POST[id_skema]' ORDER BY `kode_unit` ASC";
		$getunit2=$conn->query($sqlgetunit2);
		$noun2=1;
		$id_skemakkni=$_POST['id_skema'];
