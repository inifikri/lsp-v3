...
$rsql=$conn->query("SELECT * FROM `pengusul` WHERE `no_pendaftaran`='$_POST[username]'");
$r=$rsql->fetch_assoc();
...
if ($pass_lama==$_POST[password]){
  ...
  if ($_POST[pass_baru]==$_POST[pass_ulangi]){
    $conn->query("UPDATE `pengusul` SET `password` = '$pass_baru' WHERE `no_pendaftaran`='$_POST[username]'");
