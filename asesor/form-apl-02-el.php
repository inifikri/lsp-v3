...
while ($unk=$getunitkompetensi->fetch_assoc()){
	...
	while ($el=$getelemen->fetch_assoc()){
		...
		$getcatatanbukti="SELECT * FROM `asesi_apl02` WHERE `id_asesi`='$_GET[ida]' AND `id_elemen`='$el[id]' AND `id_skemakkni`='$sq[id]' AND `id_jadwal`='$_GET[idj]'";
		...
		$write->easyCell($cbkt['keterangan_bukti'], 'align:L; valign:T; rowspan:'.$jumkukelemen.'; border:TR;');
		$write->printRow();
		//judul kriteria
...
$sqlcekttdasesiapl01 = "SELECT * FROM `logdigisign` WHERE `nama_dokumen`='FR.APL.02. ASESMEN MANDIRI' AND `penandatangan`='$as[nama]' AND id_skema='$jdq[id_skemakkni]' AND id_asesi='$_GET[ida]' ORDER BY `id` DESC";
...
$write->easyCell('Tanda Tangan dan Tanggal:'.$tgl_cetak, 'align:L; border:TR; font-style:B;');
$write->printRow();
$write->rowStyle('min-height:20');
...
// tandatangan asesor
$urltandatanganas=$iden['url_domain']."/asesor/media.php?module=form-apl-02-el&amp;ida=".$as['no_pendaftaran']."&amp;idj=".$jdq['id'];
...
// UPDATE @FHM-PUSTI 2 AGUSTUS 2023
$sqlcekttdasesiapl01as="SELECT * FROM `logdigisign` WHERE `nama_dokumen`='FR.APL.02. ASESMEN MANDIRI' AND `penandatangan`='$asr[nama]' AND id_skema='$jdq[id_skemakkni]' AND id_asesi='$_SESSION[namauser]' ORDER BY `id` DESC";
