...
$write->printRow();
$nomorsurattugas="No. ".$as['no_surattugas'];
...
$sqlasesituk="SELECT * FROM `asesor_verifikatortuk` WHERE `id_jadwal`='$_GET[idj]' ORDER BY `id_asesor` ASC";
...
$write->easyCell(':');
$write->easyCell('1.');
$write->easyCell('Melakukan Verifikasi Tempat Uji Kompetensi (TUK)');
$write->printRow();
$write->endTable(0);
...
if(!empty($as['tgl_surattugas'])){
	$tglsknya=tgl_indo($as['tgl_surattugas']);
}else{
...
//output file pdf
$fileoutputnya="surat-tugas-Verifikasi-TUK-oleh-asesor-".$skemakkni."-".$_GET['idj'].".pdf";
