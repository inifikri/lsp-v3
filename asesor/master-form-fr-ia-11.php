...
while ($unk=$getunitkompetensi->fetch_assoc()){
	if ($noku==1){
		$write->easyCell('Unit Kompetensi', 'align:C; rowspan:2; border:LTR');
		...
		if ($jumunitnya0==$noku){
			$write->easyCell('', 'align:C; rowspan:2; border:LBR');
		}else{
			$write->easyCell('', 'align:C; rowspan:2; border:LR');
		}
		$write->easyCell('Kode Unit', 'align:L; font-size:12; border:LTBR');
		$write->easyCell(':', 'align:C; font-size:12; border:LTBR');
		$write->easyCell($unk['kode_unit'], 'align:L; font-size:12; border:LTBR');
...
//output file pdf
$fileoutputnya="Master-FR-IA-11-".$skemakkni.".pdf";
