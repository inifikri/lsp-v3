...
$passwordnya=$conn->query("SELECT * FROM `asesor` WHERE `no_ktp`='$_POST[username]'");
$r=$passwordnya->fetch_assoc();
...
if ($pass_lama==$_POST['password']){
  ...
  if ($_POST['pass_baru']==$_POST['pass_ulangi']){
    $conn->query("UPDATE `asesor` SET `password` = '$pass_baru' WHERE `no_ktp`='$_POST[username]'");
