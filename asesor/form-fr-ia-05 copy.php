...
while ($unkb=$getunitkompetensib->fetch_assoc()){
	...
	while ($gpp=$getpertanyaan->fetch_assoc()){
		if (!empty($pertanyaan=$gpp['pertanyaan'])){
			...
			if (!$any_table) {
				$write->easyCell($pertanyaan, 'align:L; valign:T; font-size:12; colspan:2; border:TRB');
				$write->printRow();
			}
			...
			$write->easyCell($gpp['pilihan_a'], 'align:L; valign:T; font-size:12; border:R'.($any_table ? '' : 'T'));
			$write->printRow();
