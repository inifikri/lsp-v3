...
$sqlasesi="SELECT * FROM `asesor_verifikatortuk` WHERE `id_jadwal`='$_GET[idj]'";
...
$write->easyCell('CEKLIST VERIFIKASI TEMPAT UJI KOMPETENSI (TUK)', 'font-style:B,U; align:C;');
$write->printRow();
$write->endTable(5);
...
$write=new easyTable($pdf, '{10, 40, 55, 15, 15, 15, 40}', 'width:190; align:L; font-size:10;font-family:arial;');
...
$write->printRow();
$write->easyCell('Baik','align:C; bgcolor:#C6C6C6; font-style:B; border:LTB');
$write->easyCell('Rusak','align:C; bgcolor:#C6C6C6; font-style:B; border:LTB');
$write->printRow();

$sqlgetpersyaratantuk="SELECT * FROM `skema_persyaratantuk` WHERE `id_skemakkni`='$jdq[id_skemakkni]' ORDER BY `id` ASC";
...
while ($stuk=$getpersyaratantuk->fetch_assoc()){
	...
	$write->easyCell($stuk['perlengkapan'],'align:L; border:LTB');
	...
	$sqlcekceklist="SELECT * FROM `skema_ceklisvertuk` WHERE `id_jadwal`='$jdq[id]' AND `id_skemakkni`='$jdq[id_skemakkni]' AND `id_perlengkapan`='$stuk[id]'";
	$cekceklist=$conn->query($sqlcekceklist);
	$clx=$cekceklist->fetch_assoc();
	$write->easyCell($clx['jumlah'],'align:C; border:LTB');
	$write->easyCell($clx['baik'],'align:C; border:LTB');
	$write->easyCell($clx['rusak'],'align:C; border:LTB');
	$write->easyCell($clx['keterangan'],'align:L; border:LTBR');
	$write->printRow();
	$nostuk++;
}
$write->endTable(0);
...
while ($astuk=$asesituk->fetch_assoc()){
	...
	$sqlcekttdasesiapl01as="SELECT * FROM `logdigisign` WHERE `nama_dokumen`='FORM CEKLIST VERIFIKASI TUK' AND `penandatangan`='$asr[nama]' AND `url_ditandatangani`='$urltandatanganas' ORDER BY `id` DESC";
