...
CL (Ceklis Observasi/ Lembar Periksa), DIT (Daftar Instruksi Terstruktur), DPL (Daftar Pertanyaan Lisan), DPT (Daftar Pertanyaan Tertulis), VP (Verifikasi Portofolio), CUP (Ceklis Ulasan Produk), PW (Pertanyaan Wawancara)', 'align:C; valign:M; colspan:6; border:LTBR; font-style:B');
	...
	while ($gel=$getel->fetch_assoc()){
		...
		while ($kuk=$getkuk->fetch_assoc()){
			if (!empty($_GET['kand'])){
				$sqlgetskemamapa1b="SELECT * FROM `skema_mapa1b` WHERE `id_skemakkni`='$_GET[idsk]' AND `profil_kandidat`='$_GET[kand]' AND `id_unitkompetensi`='$unk[id]' AND `id_elemenkompetensi`='$gel[id]' AND `id_kriteria`='$kuk[id]'";
			}else{
				$sqlgetskemamapa1b="SELECT * FROM `skema_mapa1b` WHERE `id_skemakkni`='$_GET[idsk]' AND `id_unitkompetensi`='$unk[id]' AND `id_elemenkompetensi`='$gel[id]' AND `id_kriteria`='$kuk[id]'";
			}
			$getskemamapa1b=$conn->query($sqlgetskemamapa1b);
			$smapa1b=$getskemamapa1b->fetch_assoc();
			//cek apakah ada bukti tambahan
			if (!empty($smapa1b['ket_bukti2'] || $smapa1b['metode3t'] || $smapa1b['bukti_T2'])){
				$write->easyCell($noel.'.'.$nokuk.'.', 'align:C; valign:T; border:LTBR; font-size:10; rowspan:2');
				if (empty($kuk['kriteria_pasif'])){
					$write->easyCell($kuk['kriteria'], 'align:L; valign:T; border:LTBR; font-size:10; rowspan:2');
				}else{
					$write->easyCell($kuk['kriteria_pasif'], 'align:L; valign:T; border:LTBR; font-size:10; rowspan:2');
				}
				//
				$write->easyCell($smapa1b['ket_bukti'], 'align:L; border:LTBR; font-size:10');
				...
				$write->easyCell($smapa1b['ket_bukti2'], 'align:L; border:LTBR; font-size:10');
				$write->easyCell($smapa1b['bukti_L2'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['bukti_TL2'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['bukti_T2'], 'align:C; border:LTBR; font-style:B; font-size:10');
				//
				$write->easyCell($smapa1b['metode1t'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['metode2t'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['metode3t'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['metode4t'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['metode5t'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->easyCell($smapa1b['metode6t'], 'align:C; border:LTBR; font-style:B; font-size:10');
				$write->printRow();
				...
				if (empty($kuk['kriteria_pasif'])){
					...
					$write->easyCell($kuk['kriteria_pasif'], 'align:L; border:LTBR; font-size:10');
				}
				$write->easyCell($smapa1b['ket_bukti'], 'align:L; border:LTBR; font-size:10');
				//
				$write->easyCell($smapa1b['bukti_L'], 'align:C; border:LTBR; font-style:B; font-size:10');
...
//output file pdf
$fileoutputnya="Master-FR-MAPA-01-".$skemakkni."-(kandidat-".$_GET['kand'].").pdf";
