...
$path_asesi = '../' . $ttdxasesi['file'];
if (!empty($ttdxasesi['file']) && file_exists($path_asesi)) {
    $write->easyCell(tgl_indo($ttdxasesi['waktu']), 'img:'.$path_asesi.',h20,border:LTRB;');
} else {
    $write->easyCell(tgl_indo($ttdxasesi['waktu']), 'border:LTRB;'); // fallback
}
...
if (!empty($ttdx['file']) && file_exists($ttdx['file'])) {
    ...
    $write->easyCell(tgl_indo($ttdx['waktu']), 'border:LTRB;'); // fallback tanpa gambar
}
