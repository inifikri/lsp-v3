...
$write->printRow();
$nomorsurattugas="No. ".$jdq['no_surattugaskomtek'];
$write->easyCell($nomorsurattugas, 'align:C;');
$write->printRow();
$write->endTable(5);
$namakota=trim($wil2b['nm_wil']);
$tglcetakkota = ucwords(strtolower($namakota)).", ".$tgl_cetak;
$tglsuratkomite=date('Y-m-d', strtotime('+1 days', strtotime($jdq['tgl_asesmen'])));
$tanggal = $tglsuratkomite;
$tgl_pleno = tgl_indo($tanggal);
...
$write->easyCell(':');
$write->easyCell('1.');
$write->easyCell('Rapat Pleno Pengambilan Keputusan Asesmen oleh Komite Teknis');
$write->printRow();
$write->endTable(0);
...
$write->easyCell('');
$write->easyCell('3.');
$write->easyCell('Tempat Pleno Komite');
$write->easyCell(':');
...
$write->easyCell(':');
$tglpelaksanaan=$dayList[$day]."/ ".$tgl_pleno;
...
$write->easyCell('');
$write->easyCell('8.');
$write->easyCell('Membuat berita acara keputusan komite teknis setelah selesai melaksankan tugas paling lambat 3 (tiga) hari kerja setelah selesai melaksanakan tugas.');
$write->printRow();
$write->easyCell('');
$write->easyCell('');
$write->easyCell('9.');
