		...
		if (!empty($gpp['pertanyaan'])){
			...
			switch ($jjw['rekomendasi']){
				...
				case "BK":
					...
					$write->easyCell('', 'img:../images/checked.jpg, w5, h5; align:C; valign:T; border:TRBR;');
					$write->printRow();
				break;
				default:
					$write->easyCell('', 'img:../images/unchecked.jpg, w5, h5; align:C; valign:T; border:TRB;');
...
'.$jjw['tanggapan'], 'align:L; valign:T; font-size:11; font-style:B; colspan:2; border:LTBR');
...
'.$getk['umpan_balik_ia03'], 'align:L; valign:T; font-size:11; font-style:B; colspan:4; border:LTBR');
	$write->printRow();
	$write->endTable(5);
	...
	$write->easyCell($tgl_cetak, 'align:L; border:TR; font-style:T;');
	$write->easyCell($tgl_cetak, 'align:L; border:TR; font-style:T;');
	$write->printRow();
...
$urltandatangan=$iden['url_domain']."/media.php?module=form-ia-03&amp;ida=".$as['no_pendaftaran']."&amp;idj=".$jdq['id'];
$sqlcekttdasesiapl01="SELECT * FROM `logdigisign` WHERE `nama_dokumen`='FR.IA.03. PERTANYAAN UNTUK MENDUKUNG OBSERVASI' AND `penandatangan`='$as[nama]' AND `url_ditandatangani`='$urltandatangan' ORDER BY `id` DESC";
...
// tandatangan asesor
$urltandatanganas=$iden['url_domain']."/asesor/media.php?module=form-fr-ia-03&amp;ida=".$as['no_pendaftaran']."&amp;idj=".$jdq['id'];
$sqlcekttdasesiapl01as="SELECT * FROM `logdigisign` WHERE `nama_dokumen`='FR.IA.03. PERTANYAAN UNTUK MENDUKUNG OBSERVASI' AND `penandatangan`='$asr[nama]' AND `url_ditandatangani`='$urltandatanganas' ORDER BY `id` DESC";
...
//output file pdf
$fileoutputnya="FR-IA-03-".$skemakkni."-".$_GET['idj']."-".$_GET['ida'].".pdf";
