...
$sqlasesi="SELECT * FROM `asesor` WHERE `id`='$_GET[idas]'";
...
$sqlasesor="SELECT * FROM `asesor` WHERE `id`='$_GET[idas]'";
...
$write->printRow();
$nomorsurattugas="No. ".$jdq['no_surattugas'];
...
$tanggal2 = $jdq['tgl_asesmen_akhir'];
$day2 = date('D', strtotime($tanggal2));
$dayList2 = array(
	'Sun' => 'Minggu',
	'Mon' => 'Senin',
	'Tue' => 'Selasa',
...
$write->easyCell('Nama');
$write->easyCell(':');
$write->easyCell($namaasesor, 'font-style:B;');
$write->printRow();
$write->easyCell('Jabatan');
$write->easyCell(':');
$write->easyCell('Asesor');
$write->printRow();
$write->easyCell('No. Reg');
$write->easyCell(':');
$write->easyCell($asr['no_induk']);
$write->printRow();
$write->easyCell('Asal Instansi');
$write->easyCell(':');
$write->easyCell($asr['institusi_asal']);
$write->printRow();
$write->endTable(0);
...
$write->easyCell(':');
$write->easyCell('1.');
$write->easyCell('Mengases Kompetensi');
$write->printRow();
$write->endTable(0);
...
$write->easyCell('');
$write->easyCell('3.');
$write->easyCell('Tempat Pelaksanaan');
$write->easyCell(':');
...
$tglpelaksanaan2=$dayList2[$day2]."/ ".$tgl_cetak2;
if ($tgl_cetak==$tgl_cetak2){
	$write->easyCell($tglpelaksanaan);
}else{
	$tglpelaksanaanx=$tglpelaksanaan." sd. ".$tglpelaksanaan2;
	$write->easyCell($tglpelaksanaanx);
}
$write->printRow();
$write->easyCell('');
$write->easyCell('');
...
$write->printRow();
$write->endTable(5); */
if ($lq['nama_jabatanpimpinan']=='Direktur'){
	$namapimpinan='Direktur';
}else{
	$namapimpinan='Ketua';
}
$write=new easyTable($pdf, '{100,90}', 'width:190; align:L; font-size:10;font-family:arial;');
$write->easyCell('');
$write->easyCell($tglcetakkota);
$write->printRow();
if (!empty($lq['ttddigital'])){
	$write->easyCell('');
	...
	$write=new easyTable($pdf, '{100,90}', 'width:190; align:L; font-style:B; font-size:10;font-family:arial;');
	$write->easyCell('');
	$write->easyCell($ttdcetakketua, 'align:L;');
}else{
...
$write->endTable(0);
$pdf->AddPage('L');
$write=new easyTable($pdf, '{277}', 'width:277; font-size:12;font-family:arial;');
$write->easyCell('Lampiran : Surat '.$nomorsurattugas, 'align:L;');
$write->printRow(5);
$write->easyCell('DAFTAR PESERTA ASESMEN', 'align:C;  font-style:B;');
$write->printRow();
$write->endTable(5);
...
$write->printRow();
$write->easyCell('Skema Kompetensi','font-style:B');
$write->easyCell(':','font-style:B');
$write->easyCell($skemakkni,'font-style:B');
$write->printRow();
$write->endTable(0);
