...
if ($jumcert>0){
	...
	$sqlgetnoregsert0a="SELECT * FROM `sertifikat` WHERE `id_asesi`='$as[no_pendaftaran]' AND `id_jadwal`='$jdq[id]'";
...
$write->printRow();
$write->rowStyle('min-height:10');
if (strlen($jabatankerja)>39){
	$write->easyCell($jabatankerja, 'line-height:0.75; font-size:12; font-style:B; align:C; valign:B; colspan:3;');
	$write->printRow();
	$write->easyCell($workposition, 'line-height:0.75; font-size:11; font-style:BI; align:C; valign:T; colspan:3;');
	$write->printRow();
}else{
	$write->easyCell($jabatankerja, 'line-height:0.75; font-size:16.5; font-style:B; align:C; valign:B; colspan:3;');
	$write->printRow();
	$write->easyCell($workposition, 'line-height:0.75; font-size:15.5; font-style:BI; align:C; valign:T; colspan:3;');
	$write->printRow();
}
$write->rowStyle('min-height:10');
...
if ($jumunit>10){
	if ($jumunit>30){
		...
		$conn->query("TRUNCATE `tmpunitblangko`");
		//membuat data sementara
		while ($unkx=$getunitkompetensi->fetch_assoc()){
			if ($noxx<=$div){
				$sqlbuatdata="INSERT INTO `tmpunitblangko`(`k1`, `k2`, `k3`, `k4`) VALUES ('$noxx', '$unkx[kode_unit]', '$unkx[judul]', '$unkx[judul_eng]')";
				$conn->query($sqlbuatdata);
				$noxx++;
			}
		}
		$no=0;
		$sqlgetunitkompetensitx="SELECT * FROM `unit_kompetensi` WHERE `id_skemakkni`='$sq[id]' ORDER BY `id` ASC";
		$getunitkompetensitx=$conn->query($sqlgetunitkompetensitx);
		while ($unktx=$getunitkompetensitx->fetch_assoc()){
			$no++;
			$no2=$no-$div;
			if ($no <= $div){
				$conn->query("UPDATE `tmpunitblangko` SET `k1`='$no', `k2`='$unktx[kode_unit]', `k3`='$unktx[judul]', `k4`='$unktx[judul_eng]' WHERE `k1`='$no'");
			}else{
				$conn->query("UPDATE `tmpunitblangko` SET `k5`='$no', `k6`='$unktx[kode_unit]', `k7`='$unktx[judul]', `k8`='$unktx[judul_eng]' WHERE `k1`='$no2'");
			}
		}
		$sqlunktemp="SELECT * FROM `tmpunitblangko` ORDER BY `row` ASC";
		$getunitkompetensitemp=$conn->query($sqlunktemp);
		while ($unkt=$getunitkompetensitemp->fetch_assoc()){
			if (empty($unkt['k4']) OR $unkt['k4']=='-'){
				$write->easyCell($unkt['k1'].'.', 'font-size:7; font-style:B; align:C; valign:M; border:LTBR;');
				$write->easyCell($unkt['k2'], 'line-height:0.6; font-size:6; font-style:B; align:C; valign:M; border:LTBR;');
				$write->easyCell($unkt['k3'], 'line-height:0.6; font-size:7; font-style:B; align:L; valign:M; border:LTBR;');
				$write->easyCell($unkt['k5'].'.', 'font-size:7; font-style:B; align:C; valign:M; border:LTBR;');
				$write->easyCell($unkt['k6'], 'line-height:0.6; font-size:6; font-style:B; align:C; valign:M; border:LTBR;');
				$write->easyCell($unkt['k7'], 'line-height:0.6; font-size:7; font-style:B; align:L; valign:M; border:LTBR;');
				$write->printRow();
				...
				$write->easyCell($unkt['k1'].'.', 'font-size:7; font-style:B; align:C; valign:M; border:LTBR; rowspan:2;');
				$write->easyCell($unkt['k2'], 'line-height:0.6; font-size:6; font-style:B; align:C; valign:M; border:LTBR; rowspan:2;');
				$write->easyCell($unkt['k3'], 'line-height:0.6; font-size:7; font-style:B; align:L; valign:M; border:LTR;');
				$write->easyCell($unkt['k5'].'.', 'font-size:7; font-style:B; align:C; valign:M; border:LTBR; rowspan:2;');
				$write->easyCell($unkt['k6'], 'line-height:0.6; font-size:6; font-style:B; align:C; valign:M; border:LTBR; rowspan:2;');
				$write->easyCell($unkt['k7'], 'line-height:0.6; font-size:7; font-style:B; align:L; valign:M; border:LTR;');
				$write->printRow();
				$write->easyCell($unkt['k4'], 'line-height:0.6; font-size:6; font-style:I; align:L; valign:T; border:LBR;');
				$write->easyCell($unkt['k8'], 'line-height:0.6; font-size:6; font-style:I; align:L; valign:T; border:LBR;');
				$write->printRow();
				//$nounk++;
			}
		}
		...
		$write->easyCell('No.', 'font-size:8; font-style:B; align:C; valign:M; border:LTR; rowspan:2;');
		$write->easyCell('Kode Unit Kompetensi', 'font-size:8; font-style:B; align:C; valign:B; border:LTR;');
		$write->easyCell('Judul Unit Kompetensi', 'font-size:8; font-style:B; align:C; valign:B; border:LTR;');
		$write->printRow();
		$write->easyCell('Code of Competency Unit', 'font-size:8; font-style:BI; align:C; valign:T; border:LBR;');
		$write->easyCell('Title of Competency Unit', 'font-size:8; font-style:BI; align:C; valign:T; border:LBR;');
		$write->printRow();
		while ($unk=$getunitkompetensi->fetch_assoc()){
			if (empty($unk['judul_eng']) OR $unk['judul_eng']=='-'){
				$write->easyCell($nounk.'.', 'font-size:7; font-style:B; align:C; valign:M; border:LTBR;');
				$write->easyCell($unk['kode_unit'], 'font-size:7; font-style:B; align:C; valign:M; border:LTBR;');
				$write->easyCell($unk['judul'], 'line-height:0.6; font-size:7; font-style:B; align:L; valign:M; border:LTBR;');
				$write->printRow();
			}else{
				$write->easyCell($nounk.'.', 'font-size:7; font-style:B; align:C; valign:M; border:LTBR; rowspan:2;');
				$write->easyCell($unk['kode_unit'], 'font-size:7; font-style:B; align:C; valign:M; border:LTBR; rowspan:2;');
