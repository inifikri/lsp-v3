						...
						<div class="container">
							<div class="row">
								<div class="col-md-7 contact-form">
				    				<div class="heading">
										<h3 class="uppercase head-6"><span class="main-color">Pendaftaran</span> Akun Asesi <br>Oleh Pengusul Instansi/ Institusi</h3>
									</div>
									<?php
		                		$sql="SELECT * FROM `data_wilayah`";
						echo "<form action='pendaftaran-ref.php' method='post'>
							...
							<div class='form-input'>
				    				<label>Anda diusulkan oleh Instansi/ Institusi :<span class='red'>*</span></label>
								<select name='id_pengusul' class='form-control shape'  id='id_pengusul'>";
								$sqlpengusul="SELECT * FROM `pengusul` WHERE `id`='$_GET[idp]'";
								$pengusul=$conn->query($sqlpengusul);
								while($pngs=$pengusul->fetch_assoc()){
									echo "<option value='$pngs[no_pendaftaran]'>$pngs[nama]-$pngs[nama_kantor] ($pngs[no_pendaftaran])</option>";
								}
								echo"</select>
							</div>
